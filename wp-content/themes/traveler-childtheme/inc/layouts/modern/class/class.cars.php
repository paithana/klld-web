    ...
    class STCars extends TravelerObject
    {
        ...
        public function st_format_car_price(){
            ...
            if (isset($_REQUEST['location_id']) && !empty($_REQUEST['location_id'])) {
                $locations = get_post_meta($post_id, 'multi_location', true);
                if (empty($locations)) {
                    $pass_validate = false;
            ...
            if (!empty($check_in_n) && !empty($check_out_n)) {
                ...
                if ($period < $booking_period) {
                    ...
                    wp_send_json(array('message' =>sprintf(__('Booking must be made %d day(s) before the pickup date', 'traveler'), $booking_period) ));
                }
                $unit = st()->get_option('cars_price_unit', 'day');
            ...
            $html .= '<div id="total-text"><h5>' . __('Total','traveler') . '</h5></div>
					<div id="total-value">
						<div class="st-price-origin">
							<span class="price"> ' . esc_html($price_new_html) . '</span>
						</div>
					</div>';
            $data = [
                'price' => $sale_price,
                ...
                while ($query->have_posts()) {
                    ...
                    if(function_exists('check_using_elementor') && check_using_elementor()){
                        ...
                        if ($version == 'elementorv2') {
                            $classes = '';
                            if($layoutRequest == 3){
                                $classes = ($st_style == 'grid') ? 'col-12 col-sm-6 col-md-6 col-lg-4 item-service' : 'col-12 col-sm-12 col-md-12 col-lg-12 item-service';
                            }else{
        ...
        function do_add_to_cart()
        {
            ...
            if (!empty($check_in_n) && !empty($check_out_n)) {
                ...
                if ($period < $booking_period) {
                    STTemplate::set_message(sprintf(__('Booking must be made %d day(s) before the pickup date', 'traveler'), $booking_period), 'danger');
                    $pass_validate = false;
                    return false;
                }
                $unit = st()->get_option('cars_price_unit', 'day');
        ...
        function add_type_widget_func()
        {
            ...
            $data_value = sanitize_text_field( $_REQUEST['data_value'] );
            $data_json = sanitize_text_field( $_REQUEST['data_json'] );
            $data_title_filter = sanitize_text_field( $_REQUEST['title_filter'] );
        ...
        static function get_info_price($post_id = null, $date_start = false, $date_end = false, $pick_up = false, $drop_off = false)
        {
            ...
            $price_origin = (int)get_post_meta($post_id, 'cars_price', true);
            $list_price = [];
            $price = $price_origin;
