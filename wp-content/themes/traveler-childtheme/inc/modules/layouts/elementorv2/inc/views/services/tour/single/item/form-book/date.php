...
// echo "<!--pre>";
// var_dump( TourHelper::checkAvailableTour(the_ID(), strtotime($start), strtotime($end)););
// echo "</pre-->";
...
// $new_start = date('d/m/Y',strtotime($start));
// $new_start = date('m/d/Y h:i a',TravelHelper::convertDateFormat(STInput::request('start')));
// $new_end = date('m/d/Y h:i a',TravelHelper::convertDateFormat(STInput::request('end')));
// 
// $date = STInput::request('date',date('d/m/Y h:i a'))
$date     = STInput::request( 'date', date( 'd/m/Y h:i a' ) . '-' . date( 'd/m/Y h:i a', strtotime( '+1 day' ) ) );
if (TravelHelper::getDateFormat() === 'd/M/Y') {
    //  date('M/d/Y', strtotime(str_replace('/', '-', $start)));
    $time_checkin =  strtotime(date('m/d/Y', strtotime(str_replace('/', '-', $start))));
    $time_checkout = strtotime(date('m/d/Y', strtotime(str_replace('/', '-', $end))));

    $date = STInput::request('date',date('d/m/Y h:i a',$time_checkin). '-' . date('d/m/Y h:i a', $time_checkout));
    // echo "<!--pre>";
    // var_dump($time_checkin);
    // echo "</pre-->";
