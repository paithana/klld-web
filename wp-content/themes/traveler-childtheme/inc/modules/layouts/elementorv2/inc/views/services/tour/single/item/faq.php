<?php $tours_faq = get_post_meta(get_the_ID(), 'tours_faq', true);
if(!empty($tours_faq)){
?>
<div class="st-hr"></div>
<h2 class="st-heading-section" id="st-faq">
    <?php echo __('General Information', 'traveler'); ?>
</h2>
<div class="st-faq-list">
    <?php
        $i = 0;
        foreach ($tours_faq as $k => $v){
            $section_id = strtolower(trim(preg_replace('/[^A-Za-z0-9-]+/', '-', esc_html($v['title'])), '-'));
            ?>
