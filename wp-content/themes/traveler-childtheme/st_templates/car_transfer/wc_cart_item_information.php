<?php
/**
 * Car Transfer Cart Item Information - Child Theme Override
 * Shows separate arrival and departure times for transfers
 */
...
// Get time values
$pick_up_time = isset($st_booking_data['pick_up_time']) ? $st_booking_data['pick_up_time'] : '';
$drop_off_time = isset($st_booking_data['drop_off_time']) ? $st_booking_data['drop_off_time'] : '';
$is_roundtrip = !empty($st_booking_data['roundtrip']) && $st_booking_data['roundtrip'] === 'yes';
?>

<p class="booking-item-description">
    <?php // Arrival/Pick-up ?>
    <strong><?php echo __('Arrival:', 'traveler'); ?></strong>
    <?php echo date_i18n($format, $pick_up_date); ?>
    <?php if (!empty($pick_up_time)): ?>
        <?php echo esc_html($pick_up_time); ?>
    <?php else: ?>
        <?php echo date_i18n('H:i A', $pick_up_date); ?>
    <?php endif; ?>
    <br/>
    
    <?php if ($is_roundtrip): ?>
        <?php // Departure/Drop-off for roundtrip ?>
        <strong><?php echo __('Departure:', 'traveler'); ?></strong>
        <?php echo date_i18n($format, $drop_off_date); ?>
        <?php if (!empty($drop_off_time)): ?>
            <?php echo esc_html($drop_off_time); ?>
        <?php else: ?>
            <?php echo date_i18n('H:i A', $drop_off_date); ?>
        <?php endif; ?>
        <br/>
    <?php endif; ?>
