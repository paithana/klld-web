...
<div class="row st-woo-cartpage">
    <div class="col-md-8 col-left">
        <form action="<?php echo esc_url(wc_get_cart_url()); //echo esc_url( WC()->cart->get_cart_url() ); ?>"
            ...
            ?>
            <h2 class="st-woo-heading"><?php echo __('Your Bookings', 'traveler'); ?></h2>
                ...
                foreach (WC()->cart->get_cart() as $cart_item_key => $cart_item) {
                    ...
                    if ($_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters('woocommerce_cart_item_visible', true, $cart_item, $cart_item_key)) {
                        if($st_booking_data['st_booking_post_type'] === 'car_transfer'){
                            $product_title = $st_booking_data['title_cart'];
                        }
                        else{
                        ...
                        <li class="<?php echo esc_attr(apply_filters('woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key)); ?>">
                            ...
                            if($post_type == "car_transfer"){
                                echo '<span class="booking-item-wishlist-title"><i class="fa fa-flag-o"></i> ' . __('transfer', 'traveler') . ' <span></span></span>';
                            }
                            else{
...
', esc_url(wc_get_cart_remove_url($cart_item_key)), __('Remove this item', 'traveler')), $cart_item_key);
                                    ...
                                    <div class="col-sm-3">
                                        ...
                                        if (isset($cart_item['st_booking_data']) and !empty($cart_item['st_booking_data'])) {
                                            ...
                                            if(isset($st_booking_data['st_booking_post_type']) && $st_booking_data['st_booking_post_type'] == 'st_hotel' && $hotel_alone_in_setting == $booking_id){
                                                printf('<a href="%s" target="_blank">%s</a>', get_permalink($st_booking_data['room_id']), get_the_post_thumbnail($st_booking_data['room_id'], 'medium', array('alt' => TravelHelper::get_alt_image(get_post_thumbnail_id($st_booking_data['room_id'])))));
                                            }else{
                                                if($st_booking_data['st_booking_post_type'] === 'car_transfer'){
                                                    printf('<a href="%s" target="_blank">%s</a>', '#', get_the_post_thumbnail($booking_id, 'medium', array('alt' => TravelHelper::get_alt_image(get_post_thumbnail_id($booking_id)))));
                                                } else {
                                                    printf('<a href="%s" target="_blank">%s</a>', get_permalink($booking_id), get_the_post_thumbnail($booking_id, 'medium', array('alt' => TravelHelper::get_alt_image(get_post_thumbnail_id($booking_id)))));
                                                }
                                            }
    ...
    <div class="col-md-4 col-right">
        <h2 class="st-woo-heading"><?php _e( 'Booking totals', 'traveler' ); ?></h2>
