<?php
/**
 * Review order table - Child Theme Override (Modern Layout)
 *
	...
	foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
		...
		if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_checkout_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
			...
			?>
				<header class="clearfix">
					<div class="col-left">
						<h5 class="booking-item-payment-title">
							...
							} elseif ( isset( $cart_item['st_booking_data'] ) && isset( $cart_item['st_booking_data']['st_booking_post_type'] ) && $cart_item['st_booking_data']['st_booking_post_type'] == 'st_hotel' ) {
								$st_booking_data = $cart_item['st_booking_data'];
							...
							if ( isset($cart_item['st_booking_data']['st_booking_post_type']) && $cart_item['st_booking_data']['st_booking_post_type'] == 'car_transfer' ) {
					...
					<a class="booking-item-payment-img" target="_blank" href="<?php echo esc_url( $product_url ); ?>">
						...
						if ( isset( $cart_item['st_booking_data'] ) and ! empty( $cart_item['st_booking_data'] ) ) {
							...
							$hotel_alone_in_setting = st()->get_option( 'hotel_alone_assign_hotel', '' );
							if ( isset( $st_booking_data['st_booking_post_type'] ) && $st_booking_data['st_booking_post_type'] == 'st_hotel' && $hotel_alone_in_setting == $booking_id ) {
								echo get_the_post_thumbnail( $st_booking_data['room_id'], 'thumbnail', [ 'alt' => TravelHelper::get_alt_image( get_post_thumbnail_id() ) ] );
							} else {
								echo get_the_post_thumbnail( $booking_id, 'thumbnail', [ 'alt' => TravelHelper::get_alt_image( get_post_thumbnail_id() ) ] );
							}
						} else {
				...
				<?php
				// FIXED: Display booking details for EACH item INSIDE the loop
				if ( isset( $cart_item['st_booking_data'] ) && ! empty( $cart_item['st_booking_data'] ) ) {
					$st_booking_data_item = $cart_item['st_booking_data'];
					$st_booking_data_item['cart_item_key'] = $cart_item_key;
					$item_post_type = isset( $st_booking_data_item['st_booking_post_type'] ) ? $st_booking_data_item['st_booking_post_type'] : false;
					$item_booking_id = isset( $st_booking_data_item['st_booking_id'] ) ? $st_booking_data_item['st_booking_id'] : false;
					
					if ( $item_post_type && $item_booking_id ) {
						do_action( 'st_wc_cart_item_information_' . $item_post_type, $st_booking_data_item );
					}
				}
				?>
