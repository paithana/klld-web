        ...
        <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="dropdown-mini-cart">
            ...
            if (!empty($items)) :
                foreach ($items as $item => $values) :
                    ...
                    // custom name on cart topbar-items
                    if (get_post_type($post_id) == 'st_cars') {
                        $car_post_meta = $values['st_booking_data'];
                        $post_title = $car_post_meta['title_cart'];
                    }
                    $quantity = (int) $values['quantity'];
                    $price = (float) $values['line_total'];
                    $tax = (float) $values['line_tax'];
                    $price = $price + $tax;
                    // var_dump($values['st_booking_data']);
            ?>
                    <li class="cart-item">
                        <div class="media d-flex align-items-top">
                            <div class="media-left">
                                ...
                                if (has_post_thumbnail($post_id)) {
                                    $car_post_meta = $values['st_booking_data'];
                                    if($car_post_meta['st_booking_post_type'] == 'car_transfer'){
                                        $post_id = $car_post_meta['car_id'];
                                    }
                            ...
                            <div class="media-body  ms-3">
                                ...
                                ?>
                                    <h4 class="media-heading"><a class="st-link c-main" href="<?php echo get_the_permalink($room_id) ?>"><?php echo esc_html($post_title); ?></a>
                                    </h4>
                                <?php else : ?>
                                    <h4 class="media-heading"><a class="st-link c-main" href="<?php echo get_the_permalink($post_id) ?>"><?php echo esc_html($post_title); ?></a>
                                    </h4>
                                <?php endif; ?>
                ...
                ?>
                <li class="cart-total">
                    <div class="sub-total"><?php echo __('Subtotal', 'traveler') ?> <span class="price"><?php echo balanceTags($cart_total_amount); ?></span>
                    </div>
                    <a href="<?php echo esc_url(get_permalink(wc_get_page_id('cart'))) ?>" class="btn btn-green btn-full upper"><?php echo __('View Cart', 'traveler') ?></a>

                    <a href="<?php echo add_query_arg(['action' => 'st-remove-cart', 'security' => wp_create_nonce('st-security')]);
                                    ?>"
                       class="btn btn-cart-remove btn-full upper mt10" style="background-color: #ffcc00; color: #38761d;">
                        <?php echo __('Clear Cart', 'traveler')
                        ?>
                    </a>
                    <a href="<?php echo esc_url(get_permalink(wc_get_page_id('checkout'))) ?>" class="btn btn-full btn-cart-checkout upper mt10"><?php echo __('Check Out', 'traveler') ?></a>
                </li>
            <?php
            else :
            ?>
    ...
    <li class="dropdown dropdown-minicart">
        ...
        <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="dropdown-mini-cart">
            ...
            ?>
                        ...
                        <a href="<?php echo add_query_arg(['action' => 'st-remove-cart', 'security' => wp_create_nonce('st-security')]); ?>" class="cart-delete-item"><i class="fa">
                                <svg width="16px" height="16px" viewBox="0 0 16 16" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                                    <g id="Menu" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd" stroke-linecap="round" stroke-linejoin="round">
                                        <g id="Menu-Mega" transform="translate(-1355.000000, -383.000000)" stroke="#A0A9B2">
                                                ...
                                                <g id="hotel" transform="translate(0.000000, 198.000000)">
                                                    <g id="Group" transform="translate(236.000000, 56.000000)">
                                                        <g id="bin-1">
                                                            ...
                                                            <path d="M8.5,0 L5.5,0 C4.94771525,0 4.5,0.44771525 4.5,1 L4.5,2 L9.5,2 L9.5,1 C9.5,0.44771525 9.05228475,0 8.5,0 Z" id="Shape"></path>
                                                            <path d="M5.5,10.5 L5.5,5.5" id="Shape"></path>
                                                            <path d="M8.5,10.5 L8.5,5.5" id="Shape"></path>
                                                            <path d="M11.5766667,13.0826667 C11.5336578,13.6011549 11.100269,14.0000465 10.58,14 L3.42066667,14 C2.9003977,14.0000465 2.4670089,13.6011549 2.424,13.0826667 L1.5,2 L12.5,2 L11.5766667,13.0826667 Z" id="Shape"></path>
                                                        </g>
                                                    </g>
                                                </g>
                                            </g>
                ...
                ?>
                <li class="cart-total">
                    <div class="sub-total"><?php echo __('Subtotal', 'traveler') ?> <span class="price"><?php echo TravelHelper::format_money($cart_total_amount, false); ?></span>
                    </div>
                    <a href="<?php echo esc_url($check_out_url) ?>" class="btn btn-green btn-full upper"><?php _e('Pay Now', 'traveler') ?></a>
                </li>
            <?php
            else :
            ?>
