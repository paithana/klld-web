...
<p class="booking-item-description"><span><?php echo __('Tour Date', 'traveler'); ?>: </span><?php echo esc_html(date_i18n( $format , strtotime( $data['check_in'] ) ) . ($data['starttime'] != '' ? ' - ' . $data['starttime'] : '')); ?></p>
...
<p class="booking-item-description"><span><?php echo __('Tour Date', 'traveler'); ?>: </span><?php echo esc_html(date_i18n( $format , strtotime( $data['check_in'] ) ) . ($data['starttime'] != '' ? ' - ' . $data['starttime'] : '')); ?></p>
<!--p class="booking-item-description"><span><?php //echo __('Duration', 'traveler'); ?>: </span><?php //echo esc_html(date_i18n( $format , strtotime( $data['check_out'] ) ) ); ?></p-->
<?php endif; ?>
<?php }else{ ?>
...
<div id="<?php echo esc_attr($div_id);?>" class='<?php if (apply_filters('st_woo_cart_is_collapse' , true)) {echo esc_attr("collapse");}?>'>
			...
			<p class="booking-item-description">
                ...
                <?php 
                    ...
                    if(isset($data['guest_name']) && !empty($data['guest_name'])){
                        foreach ($data['guest_name'] as $key => $value) {
                            $guest_title = $data['guest_title'][$key];
                            $guest_name = $data['guest_name'][$key];
                            echo $guest_title. " " . $guest_name ."</br>";
                        }
                    }
                ?>
			</p>
