                                ...
                                foreach ( $sort_topbar_menu as $key => $val ) {
									if ( !empty( $val[ 'topbar_item' ] ) && $val[ 'topbar_position' ] == 'left' ) {
										...
										if ( !empty( $val[ 'topbar_item' ] ) && $val[ 'topbar_position' ] == 'left' && (empty( $val[ 'topbar_is_social' ] ) || $val[ 'topbar_is_social' ] == 'off') ) {
											echo '<li class="d-none d-sm-none d-md-inline-block"><a href="' . esc_url( $val[ 'topbar_custom_link' ] ) . '" target="' . esc_attr($target) . '"><i class="' . esc_attr($icon) . '"></i>' . esc_html( $val[ 'topbar_custom_link_title' ] ) . '</a></li>';
										}
										if($val['topbar_item'] == 'shortcode') {
											echo stt_elementorv2()->loadView("components/topbar-items/shortcode", ['val' => $val] );
										}
									}
                                }
                                ?>
    ...
    <div class="header header-sticky-menu d-flex justify-content-between align-items-center">
        ...
        <div class="header-left d-flex align-items-center">
            ...
            <?php if (is_front_page()) : ?>
                <h1 class="tag_h1"><?php bloginfo( 'name' ); ?></h1>
            <?php endif; ?>
            <nav id="st-main-menu">
