...
if ( isset( $item_id ) and $item_id ) :
	...
	$hotel_package    = isset( $item['data']['package_hotel'] ) ? $item['data']['package_hotel'] : [];
	$activity_package = isset( $item['data']['package_activity'] ) ? $item['data']['package_activity'] : [];
	$car_package      = isset( $item['data']['package_car'] ) ? $item['data']['package_car'] : [];
	$flight_package   = isset( $item['data']['package_flight'] ) ? $item['data']['package_flight'] : [];
...
<div class="service-section">
	<div class="service-left">
		<h3 class="title"><a href="<?php echo get_permalink( $tour ) ?>"><?php echo get_the_title( $tour ) ?></a></h3>
		<?php
	...
	<div class="service-right">
		<?php
		echo get_the_post_thumbnail( $tour, [
			110,
			110,
			'bfi_thumb' => true,
            ], [
			'alt'   => TravelHelper::get_alt_image( get_post_thumbnail_id( $tour ) ),
...
<div class="info-section">
	<ul>
		...
		<?php
		// Pickup location
		$pickup  = $item['data']['pickup'];
		if ( $pickup ) {
			?>
			<li>
				<span class="label"><?php echo __( 'Pick up', 'traveler' ); ?></span>
				<span class="value"><?= esc_html( $pickup ); ?></span>
			</li>
			<?php
		}
		?>
			...
			<li class="package-value">
				...
				<p class="booking-item-payment-price-amount">
					...
					foreach ( $car_package as $k_hp => $v_hp ) :
						if ( ! empty( $v_hp['car_quantity'] ) && intval( $v_hp['car_quantity'] ) > 0 ) {
							?>
							<span class="pull-right">
...
<div class="total-section">
	...
	$data_price   = ! empty( $item['data']['data_price'] ) ? $item['data']['data_price'] : [];
	...
	<ul>
		<?php if ( $price_type == 'person' or $price_type == 'fixed_depart' ) { ?>
			...
			<li>
				...
				<span class="value">
					<?php
					if ( $item['data']['adult_price'] ) {
						echo TravelHelper::format_money( $item['data']['adult_price'] );
					} else {
						echo '0';
					}
					?>
			...
			<li>
				...
				<span class="value">
					<?php
					if ( $item['data']['child_price'] ) {
						echo TravelHelper::format_money( $item['data']['child_price'] );
					} else {
						echo '0';
					}
					?>
			...
			<li>
				...
				<span class="value">
					<?php
					if ( $item['data']['infant_price'] ) {
						echo TravelHelper::format_money( $item['data']['infant_price'] );
					} else {
						echo '0';
					}
					?>
		...
		<?php } else { ?>
			<li>
				...
				<span class="value">
					<?php
					if ( $base_price ) {
						echo TravelHelper::format_money( $base_price );
					} else {
						echo '0';
					}
					?>
	...
	<?php if ( is_array( $hotel_package ) && count( $hotel_package ) ) : ?>
		...
		if ( ! empty( $data_price['total_bulk_discount'] ) && $total_price_origin > $sale_price && floatval( $data_price['total_bulk_discount'] ) > 0 ) {
			?>
				<li>
		...
		if ( STCart::use_coupon() ) :
			...
			<li>
				<span class="label text-left">
					...
					<?php } else { ?>
						<a href="<?php echo st_get_link_with_search( get_permalink(st()->get_option('page_checkout')), [ 'remove_coupon' ], [ 'remove_coupon' => STCart::get_coupon_code() ] ) ?>"
