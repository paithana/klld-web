                ...
                if (st_check_service_available('st_hotel')) {
                    ...
                    if (!empty($category_hotel)) {
                        ...
                        if($term_tax_hotel[0] != 0){
							$taxonomies = get_object_taxonomies($v);
                            $arr_tax = [
                                'relation' => 'OR',
                            ];
                    ...
                    if ($query_service->have_posts()) {
                        if ($list_style == 'grid') {
                            ...
                            while ($query_service->have_posts()):
                                $query_service->the_post();
                                $html .= '<div class="' . esc_attr($col_classes) . '">';
                                $html .= apply_filters('st_elementor_loop_tour_grid_mod_service_view',stt_elementorv2()->loadView('services/tour/loop/grid') , $grid_tour_style);
                                $html .= '</div>';
                            endwhile;
                            $tour->remove_alter_search_query();
                            wp_reset_postdata();
                ...
                <ul class="nav nav-tabs d-flex align-items-center justify-content-center nav-fill-st" id="nav-tab" role="tablist">
                    <?php
                        foreach($services as $key=>$service){
                            if($key !== 'st_flight'):?>
                            <li><a class="text-center  <?php if($key== array_key_first($services) ){echo 'active';}?>"
									id="nav-list-of_service<?php echo esc_attr($service);?>-tab"
									data-bs-toggle="tab"
									data-bs-target="#nav-list-of_service<?php echo esc_attr($key);?>-<?php echo esc_attr( $id_collapse ) ?>"
                                    role="tab"
									aria-controls="nav-list-of_service<?php echo esc_attr($key);?>-<?php echo esc_attr( $id_collapse ) ?>"
        ...
        <div class="tab-content" id="nav-content-wishlist">
            ...
            foreach($services as $key=>$service){
				...
				if ( is_singular( 'location' ) ) {
					...
					$sql = "SELECT post_id FROM {$wpdb->prefix}st_location_relationships WHERE 1=1 AND location_from IN ({$location_id}) AND post_type IN ('{$key}')";
					$res = $wpdb->get_results( $sql, ARRAY_A );
					$res = array_map ( function( $re ) {
						return $re['post_id'];
					}, $res );
                ...
                ?>
                    <div class="tab-pane stt-tab-list-ofservice <?php if($key== array_key_first($services) || $active_tab==true ){echo 'active';}?>"
						id="nav-list-of_service<?php echo esc_attr($key);?>-<?php echo esc_attr( $id_collapse ) ?>"
						role="tabpanel"
