...
<div class="booking-success-notice">
    <div class="row">
        <div class="col-lg-8 col-12 col-left">
            <div class="success-icon"><span class="stt-icon stt-icon-check"></span></div>
            <div class="notice-success">
                ...
                <p class="line2"><?php _e('Booking details have been sent to: ','traveler'); ?><span><?php echo get_post_meta($order_code, 'st_email', true) ?></span></p>
            </div>
        </div>
...
<div class="row">
    <div class="col-lg-4 col-12 order-1 order-sm-2">
        <h3 class="title">
        ...
        <div class="cart-info st-border-radius">
			...
			$post_type = $key == 'car_transfer' ? $key : get_post_type($key);
            ...
            switch ($post_type) {
                case "st_hotel":
                    echo stt_elementorv2()->loadView('services/hotel/components/success-item', [
                        'order_id' => $order_code,
                        'data' => $value,
                        'key' => $key,
                        'i' => $i
                ...
                case "st_cars":
                    ...
                    break;
				case "car_transfer":
					echo st()->load_template('layouts/modern/car_transfer/success_payment_item_row', false, array('order_id' => $order_code, 'data' => $value, 'key' => $key, 'i' => $i));
					break;
                case "st_rental":
            ...
            <div class="total-section">
                ...
                if(in_array($key, $arr_item_custom_info)){
					...
					if ( get_post_meta($order_code, 'item_post_type', true) == 'car_transfer' ) {
						$price_with_tax = $data_price['price_with_tax'];
					}
                    ...
                    $item_id            = $post_type === 'st_hotel' ? $value['data']['room_id'] : $order_code;
                        ...
                        if ($post_type != 'st_flight') {
                            ...
                            if ( isset($discount_rate) && $discount_rate > 0 ) : ?>
                                <li>
									...
									switch ($post_type) {
										case "st_hotel":
											$text_discount = __('Discount/Night', 'traveler');
											break;
										case "hotel_room":
											$text_discount = __('Discount/Night', 'traveler');
											break;
										case "st_tours":
											$tour_price_by = get_post_meta($key, 'tour_price_by', true);
											if ( $tour_price_by === 'person' ) {
												$text_discount = __('Discount/Person', 'traveler');
											} else {
												$text_discount = __('Discount', 'traveler');
											}
											break;
										case "st_activity":
											$text_discount = __('Discount/Person', 'traveler');
									...
									?>
                                    <span class="label"><?php echo esc_html( $text_discount ); ?></span>
                                    <?php
                            ...
                            <?php if (!empty($extras['value']) and is_array($extras['value']) && count($extras['value']) && ($extra_price != 0)) { ?>
                                <li>
                                    <span class="label"><?php echo __('Extras Price', 'traveler'); ?></span>
                            ...
                            if(!empty($tax) && ($tax) != 0){?>
								<li>
									<?php if ( st()->get_option( 'st_tax_include_enable', 'off') == 'on' ) : ?>
										<span class="label"><?php echo __('Tax included', 'traveler'); ?></span>
									<?php else : ?>
    ...
    <div class="col-lg-8 col-12 order-2 order-sm-1">
        <h3 class="title">
