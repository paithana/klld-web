...
if ( ! function_exists( 'st_inside_body_content' ) ) {

	function st_inside_body_content() {
		if ( st()->get_option( 'gen_disable_preload' ) == 'off' ) {
			?>
			<!-- Preload -->
			<div id="bt-preload"></div>
		...
		echo st()->get_option( 'adv_inside_body_content' );
	}
}
...
function post_thumbnail_html_func( $html, $post, $thumb, $size, $attr ) {
	if ( empty( $html ) ) {
		...
		return '<img src="https://placehold.co/600x400"  class="' . $class . '"  alt="' . $alt . '"/>';
	}
	return $html;
}
function wp_get_attachment_image_attributes_func( $attr, $attachment, $size ) {
	if ( empty( $attr['src'] ) ) {
		$attr['src'] = 'https://placehold.co/600x400';
	}
