<?php
    ...
    if ( !class_exists( 'HotelHelper' ) ) {
        class HotelHelper
        {
                    ...
                    for ( $i = intval( strtotime( $check_in ) ); $i <= intval( strtotime( $check_out ) ); $i = strtotime( '+1 day', $i ) ) {
                        ...
                        foreach ( $result as $key => $date ) {
                            if ( $allow_full_day == 'on' ) {
                                ...
                                if (
                                    $i > intval( $date[ 'check_in_timestamp' ] )
                                    && $i < intval( $date[ 'check_out_timestamp' ] )
                                    && (intval( $date[ 'check_out_timestamp' ] ) != (strtotime('+1 day', $date[ 'check_in_timestamp' ])))) {
                                    $num_room += $date[ 'number_room' ];
                                }
                                ...
                                && (intval( $date[ 'check_out_timestamp' ] ) != (strtotime('+1 day', $i)))
                                ) {
                    ...
                    if ( count( $disable ) ) {
                        foreach ( $disable as $key => $num_room ) {
                            if ( intval( $num_room + $number_r ) > $number_room )
                                $list_date[] = date( 'd-m-Y', $key );
                        }
                    }
                    if ( count( $array_fist_half_day ) ) {
                        foreach ( $array_fist_half_day as $key => $num_room ) {
                            if ( intval( $num_room + $number_r ) > $number_room )
                                $list_date_fist_half_day[] = date( 'd-m-Y', $key );
                        }
                    }
                    if ( count( $array_last_half_day ) ) {
                        foreach ( $array_last_half_day as $key => $num_room ) {
                            if ( intval( $num_room + $number_r ) > $number_room )
                                $list_date_last_half_day[] = date( 'd-m-Y', $key );
                        }
                    }

                    if( is_array( $list_date ) and count( $list_date ) ){
                        return false;
                    }
                    if ( ( is_array( $list_date_fist_half_day ) and count( $list_date_fist_half_day ) ) and ( is_array( $list_date_last_half_day ) and count( $list_date_last_half_day ) ) ){
                        if(count( $list_date_fist_half_day) >= 1 && count( $list_date_last_half_day) > 1){
                            return false;
                        }
                        if(count( $list_date_last_half_day) == 1 && (strtotime($list_date_fist_half_day[0]) < strtotime($list_date_last_half_day[0]))){
                            return false;
                        }
            ...
            static function _get_full_ordered_new( $room_id, $start, $end, $hotel = false )
            {
                if ( !TravelHelper::checkTableDuplicate( 'st_hotel' ) ) return '';
				...
				WHERE
				st_booking_post_type = '{$key_post_type}' ";
                if(!$hotel){
                    $sql    .= "AND room_origin = '{$room_id}'";
                } else {
                    $sql    .= "AND st_booking_id = '{$room_id}'";
                }

				$sql    .= "AND (
					(
						check_in_timestamp >  {$date1} AND
                        check_out_timestamp <  {$date2}
					)
					OR (

						{$date1} BETWEEN check_in_timestamp
						AND check_out_timestamp
					)
					OR (
						{$date2} BETWEEN check_in_timestamp
						AND check_out_timestamp
					)
				)
				AND `status` NOT IN ('trash', 'pending', 'incomplete', 'canceled', 'cancelled', 'wc-cancelled', 'wc-refunded', 'wc-failed')";
            ...
            static function _get_disable_date()
            {
                ...
                $room_id     = esc_html( STInput::request( 'room_id' ) );
                ...
                $allow_full_day = get_post_meta( $hotel_id, 'allow_full_day', true );
                if ( !$allow_full_day || $allow_full_day == '' ) $allow_full_day = 'on';
                $year = esc_html( STInput::request( 'year' ) );
                if ( empty( $year ) ) $year = date( 'Y' );
                $month = esc_html( STInput::request( 'month' ) );
                if ( empty( $month ) ) $month = date( 'm' );
                $year1  = $year;
                $month1 = $month - 1;
                if ( $month == 1 ) {
            ...
            public function _get_availability_hotel_room()
            {
                ...
                $room_origin        = TravelHelper::post_origin( $room_id );
                $hotel_id           = intval( get_post_meta( $room_origin, 'room_parent', true ) );
                $discount_type_no_day = get_post_meta( $room_id, 'discount_type_no_day', true );
                ...
                if ( !empty( $room_available ) ) {
                    foreach ( $room_available as $kk => $vv ) {
                        if($price_by_per_person === 'on'){
                            $st_adult_price = !empty($vv[ 'adult_price' ]) ? $vv[ 'adult_price' ] : 0;
                            $st_child_price = !empty($vv[ 'child_price' ]) ? $vv[ 'child_price' ] : 0;
                            $ev = __('Adult: ', 'traveler') . TravelHelper::format_money(st_apply_discount( $st_adult_price, $discount_type, $discount )) . '<br/>';
                            $ev .= __('Child: ', 'traveler') . TravelHelper::format_money(st_apply_discount( $st_child_price, $discount_type, $discount )) . '<br/>';
