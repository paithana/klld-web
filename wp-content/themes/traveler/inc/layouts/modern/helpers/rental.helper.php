    ...
    class RentalHelper
    {
        ...
        static function _get_min_max_date_ordered_new($rental_id, $start, $end)
        {
            ...
            $sql = "SELECT
				...
				AND check_in_timestamp >= {$start}
				AND check_out_timestamp <= {$end}
				AND status IN ('complete', 'wc-completed')";
        ...
        static function _get_availability_rental_single_each_day()
        {
            ...
            if (!empty($rental_available)) {
                foreach ($rental_available as $kk => $vv) {
					...
					$price_by_per_person = get_post_meta( $rental_origin, 'rental_price_by', true );
					if($price_by_per_person === 'person'){
						...
						$data_price_rental[ $vv[ 'check_in' ] ] = $ev;
					} else {
						$data_price_rental[ $vv[ 'check_in' ] ] = TravelHelper::format_money(st_apply_discount( $vv[ 'price' ], $discount_type, $discount ));
					}
                }
            }
                        ...
                        if (in_array($d, $list_date_2)) {
                            ...
                            if ($period < $booking_period) {
                                ...
                                $return['events'][] = [
                                    ...
                                    'day' => date('d', $i),
                                    //'status' => 'available',
                                    'event'  => ( isset( $data_price_rental[$i] ) ? $data_price_rental[$i] : TravelHelper::format_money(0) ),
        ...
        static function _get_availability_rental_single_groupday()
        {
            ...
            if (is_array($data_rental) && count($data_rental)) {
                ...
                for ($i = intval($val->$check_in); $i <= intval($val->$check_out); $i = strtotime('+1 day', $i)) {
                    $in_item = false;
                    foreach ($data_rental as $key => $val) {
        ...
        static function _get_disable_date()
        {
            ...
            $rental_id = esc_html( STInput::request('rental_id') );
            $rental_origin = TravelHelper::post_origin($rental_id, 'st_rental');
                ...
                OR (
                    ...
                    AND (
                        ...
                        OR (
                            ...
                            AND {$check_out} > check_in_timestamp
                        )
                    )
                )
            )
        ...
        static function _get_maxmin_by_date($rental_id, $check_in, $check_out)
        {
            ...
            $sql = "SELECT
					...
					AND origin_id = '{$rental_id}'
					AND st_booking_post_type = 'st_rental'
					AND status NOT IN ('trash', 'canceled','cancelled' , 'wc-cancelled')";
            $result = $wpdb->get_row($sql, ARRAY_A);
            return $result;
        }
    }
