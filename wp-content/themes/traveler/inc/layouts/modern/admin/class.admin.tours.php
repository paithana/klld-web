...
if ( ! class_exists( 'STAdminTours' ) ) {
	...
	class STAdminTours extends STAdmin {
		...
		function __construct() {
			...
			add_action( 'init', [ $this, '__update_db_available_first_run' ], 99 );

			add_action( 'st_availability_cronjob', [ $this, '__cronjob_fill_availability' ] );

			add_action( 'admin_init', [ $this, '_upgradeTourTable133' ] );
			add_action( 'st_tour_type_add_form_fields', [ $this, 'st_tour_type_add_fields' ] );
			add_action( 'st_tour_type_edit_form_fields', [ $this, 'st_tour_type_edit_fields' ] );
			add_action( 'created_st_tour_type', [ $this, 'st_tour_type_save' ], 10, 1 );
			add_action( 'edited_st_tour_type', [ $this, 'st_tour_type_save' ], 10, 1 );
			add_filter( 'manage_edit-st_tour_type_columns', [ $this, 'st_tour_type_set_colums' ] );
			add_filter( 'manage_st_tour_type_custom_column', [ $this, 'st_tour_type_custom_colums' ], 10, 3 );
		...
		public function sttDashboardChangeAvatar() {
			...
			if ( ! empty( $stAttachment ) ) {
				$url       = '';
				$thumbnail = wp_get_attachment_image_url( $stAttachment, [ 80, 80 ] );
		...
		public function st_tour_type_edit_fields() {
			...
			<tr class="form-field term-group-wrap">
				...
				<td>
					...
					<div class="stt-edit-avatar">
						<?php
						$imageURL = wp_get_attachment_image_url( $imageID, [ 80, 80 ] );
						if ( ! empty( $imageURL ) ) {
							?>
		...
		public function st_tour_type_custom_colums( $content, $columns, $term_id ) {
			switch ( $columns ) {
				case 'image':
					...
					$imageUrl = wp_get_attachment_image_url( $imageID, [ 40, 40 ] );
		...
		public function __run_fill_old_order( $key = '' ) {
			...
			$orderItems = $model->where( "st_booking_post_type in ('st_tours')", false, true )
							...
							->where( "STATUS IN('complete', 'wc-completed')", false, true )->get()->result();
		...
		static function _getInfoTour() {
			...
			if ( get_post_type( $tour_id ) == 'st_tours' ) {
				...
				$content_extra_price = st()->load_template( 'user/extra/tour', 'extra', [
					'tour_id'     => $tour_id,
					'extra_price' => $extra_price,
				] );
				$max_people          = intval( get_post_meta( $tour_id, 'max_people', true ) );
				$max_people_html     = $max_people;
				if ( $max_people < 1 ) {
					$max_people = 20;
				}
				...
				$disable_adult_name    = get_post_meta( $tour_id, 'disable_adult_name', true );
				$disable_children_name = get_post_meta( $tour_id, 'disable_children_name', true );
				$disable_infant_name   = get_post_meta( $tour_id, 'disable_infant_name', true );

				if ( $disable_adult_name == 'off' || $disable_children_name == 'off' || $disable_infant_name == 'off' ) {
					$html_required_name .= '
                    <div class="guest_name_input d-none"
                        data-placeholder="' . esc_html__( 'Guest %d name', 'traveler' ) . '"
                        data-hide-adult="' . get_post_meta( $tour_id, 'disable_adult_name', true ) . '"
                        data-hide-children="' . get_post_meta( $tour_id, 'disable_children_name', true ) . '"
                        >
                        <label><span>' . esc_html__( 'Guest Name', 'traveler' ) . '</span> <span class="required">*</span></label>
                        ...
                        <script type="text/html" id="guest_name_control_item">
                            <div class="control-item mb10">
                                ...
                                <option value="mr">' . esc_html__( 'Mr', 'traveler' ) . '</option>
                                <option value="miss">' . esc_html__( 'Miss', 'traveler' ) . '</option>
                                <option value="mrs">' . esc_html__( 'Mrs', 'traveler' ) . '</option>
                                </select>
                                <input class="form-control " placeholder="" name="guest_name[]" value="">
                            </div>
                        </script>
                    </div>
                    ';
				...
				$result['max_people']         = ( $max_people_html == 0 ) ? __( 'Unlimited', 'traveler' ) : $max_people_html;
				$result['adult_html']         = $adult_html;
				$result['child_html']         = $child_html;
				$result['infant_html']        = $infant_html;
				$result['duration']           = $duration;
				$result['required_name_html'] = $html_required_name;
			}
		...
		static function _update_duplicate_data( $id, $data ) {
			...
			if ( get_post_type( $id ) == 'st_tours' ) {
				...
				if ( ! empty( $data_prices ) ) {
					...
					$new_check_in  = strtotime( 'today UTC' );
					$new_check_out = strtotime( '+90 day', $new_check_in );
					for ( $i = $new_check_in; $i <= $new_check_out; $i = strtotime( '+1 day', $i ) ) {
						ST_Tour_Availability::inst()->insertOrUpdate([
							'post_id'         => $id,
							'check_in'        => $i,
							'check_out'       => $i,
							'price'           => $base_price,
		...
		function init_metabox() {
			...
			$layout_single              = [
				...
				[
					...
					'src'   => get_template_directory_uri() . '/v2/images/layouts/tour_detail_layout_3_new.png',
				],
			...
			if ( ! check_using_elementor() ) {
				...
				$layout_single = array_merge( $layout_single, $array_modern );
			}
			if ( check_using_elementor() ) {
				$array_elementor = [
					[
				...
				$layout_single   = array_merge( $layout_single, $array_elementor );
			}
			$layout_single = apply_filters( 'st_layout_single_tour', $layout_single );
			// Room
			$this->metabox[] = [
				'id'       => 'tour_metabox',
				...
				'fields'   => [
					...
					[
						'id'        => 'list_badges',
						'label'     => __( 'Add badge', 'traveler' ),
						'desc'  => __( 'ONLY show in Single Service Page and default Modal Checkout', 'traveler' ),
						'type'      => 'list-item',
						'settings'  => [
							[
								'id'    => 'content',
					...
					[
						'label'   => __( 'Upsell', 'traveler' ),
						'id'      => 'st_upsell',
						'type' => 'upsell_select_ajax',
						'desc' => __('Select items for Upsell (ONLY for the default Modal Checkout Success Page). Will show only items which belong to the chosen Type Service', 'traveler'),
                        'multi' => true,
                        'placeholder' => __('Search item for Upsell', 'traveler')
					],
					/**
					 * version 2.7.4
					 */
					...
					[
						...
						'id'    => 'video',
						'type'  => 'text',
						'desc'  => __( 'Input youtube/vimeo url here. With youtube, use original URL - not shorten URL', 'traveler' ),
					],
					[
						'label' => __( 'Turn on/off Pick up', 'traveler' ),
						'id'    => 'enable_pickup',
						'type'  => 'on-off',
						'desc'  => __( 'Turn on/off field Pick up for tour booking', 'traveler' ),
						'std'   => 'off',
					],
					[
					...
					[
						...
						'id'        => 'adult_price',
						'type'      => 'text',
						'desc'      => __( 'Price per adult. Input number only. No decimal or thousand separator or any other character', 'traveler' ),
						'std'       => 0,
					...
					[
						...
						'desc'      => '<em>' . __( 'Notice: Must be https://...', 'traveler' ) . '</em>',
					],
					[
					...
					[
						...
						'id'           => 'max_people',
						'type'         => 'numeric-slider',
						'desc'         => __( 'Max No. People. Enter \'0\' for unlimited', 'traveler' ),
					...
					[
						...
						'choices' => [
							...
							[
								'label' => __( 'Background image with text', 'traveler' ),
								'value' => 'style4',
							],
						],
						'std'     => 'style1',
					...
					[
						...
						'label'     => __( 'Solo Tour: Overview Background Image', 'traveler' ),
						'type'      => 'upload',
						'condition' => 'st_custom_layout_new:is(7),st_custom_layout_new:is(10)',
						'operator'  => 'or',
					],
					[
						'id'        => 'tours_highlight_bg_img',
						'label'     => __( 'Solo Tour: Tour Highlight Background Image', 'traveler' ),
						'type'      => 'upload',
						'condition' => 'st_custom_layout_new:is(7),st_custom_layout_new:is(10)',
						'operator'  => 'or',
					],
					[
						...
						'label'     => __( 'Solo Tour: Related Tour Background Image', 'traveler' ),
						'type'      => 'upload',
						'condition' => 'st_custom_layout_new:is(7),st_custom_layout_new:is(10)',
						'operator'  => 'or',
					],
		...
		static function get_min_price( $post_id = null ) {
			...
			if ( $tour_price_by != 'fixed' ) {
				...
				$query_sql_min       = "SELECT min(CAST(`price` as DECIMAL(15,6))) as base_price  FROM {$table} WHERE check_in >= UNIX_TIMESTAMP(CURRENT_DATE) AND post_id = {$post_id} AND status = 'available'";
				$get_min_price_avail = $wpdb->get_results( $query_sql_min, ARRAY_A );
				$query_sql_max       = "SELECT max(CAST(`price` as DECIMAL(15,6))) as base_price  FROM {$table} WHERE check_in >= UNIX_TIMESTAMP(CURRENT_DATE) AND post_id = {$post_id} AND status = 'available'";
				$get_max_price_avail = $wpdb->get_results( $query_sql_max, ARRAY_A );
			...
			if ( isset( $get_min_price_avail ) ) {
				...
				if ( $tour_price_by != 'fixed' ) {
					...
					$price_base_min = $price_min_max['min']['base_price'];
				}
			...
			if ( $tour_price_by == 'fixed' ) {
				...
				if ( isset( $base_price ) && ( $price_base_min < $base_price ) ) {
					$prices[] = $price_base_min;
				} else {
					$prices[] = $base_price;
				}
			} else {
				...
				} elseif ( empty( $price_adule ) && empty( $price_child ) ) {
		...
		// from 1.2.4
		static function meta_update_min_price( $post_id ) {
		...
		function meta_update_sale_price( $post_id ) {
			...
			$post_type = get_post_type( $post_id );
			if ( $post_type == 'st_tours' ) {
				$sale_price       = (float)get_post_meta( $post_id, 'adult_price', true );
				$discount         = get_post_meta( $post_id, 'discount', true );
				$is_sale_schedule = get_post_meta( $post_id, 'is_sale_schedule', true );
				$tour_price_type  = get_post_meta( $post_id, 'tour_price_by', true );
				if ( $tour_price_type == 'fixed' ) {
					...
					if ( $discount ) {
						$sale_price = $base_price - ( $base_price / 100 ) * $discount;
					}
				} else {
					if ( $is_sale_schedule == 'on' ) {
						$sale_from = get_post_meta( $post_id, 'sale_price_from', true );
		...
		function _save_booking( $order_id ) {
			...
			if ( $this->_check_validate() ) {
				...
				do_action( 'st_admin_edit_booking_before_update_status', $item_data['status'], $order_id );
				...
				if ( TravelHelper::checkTableDuplicate( 'st_tours' ) ) {
					...
					$table_tour_availability = $wpdb->prefix . 'st_tour_availability';

					$booked             = ( $_POST['adult_number'] + $_POST['child_number'] + $_POST['infant_number'] );
					$post_id            = st_get_order_by_order_item_id( $order_id )['origin_id'];
					...
					if ( $status_order != 'complete' && $_POST['status'] == 'complete' ) {
						$sql = $wpdb->prepare( "UPDATE {$table_tour_availability} SET number_booked = IFNULL(number_booked, 0) + %d WHERE post_id = %d AND check_in = %s", $booked, $post_id, $check_in_timestamp );
						$wpdb->query( $sql );
					} elseif ( $status_order == 'complete' && $_POST[ 'status' ] != 'complete' ) {
                        $sql = $wpdb->prepare( "UPDATE {$table_tour_availability} SET number_booked = GREATEST( 0, IFNULL(number_booked, 0) - %d ) WHERE post_id = %d AND check_in = %s", $booked, $post_id, $check_in_timestamp );
                        $wpdb->query($sql);
                    }
		...
		function add_col_content( $column_name, $post_ID ) {
			if ( $column_name == 'tour_layout' ) {
				...
				$layout      = get_post_meta( $post_ID, 'st_custom_layout_new', true );
				$data_layout = self::data_layout();

				echo esc_html( $data_layout[ $layout ] );
			}
				...
				if ( $tour_price_type == 'fixed' ) {
					...
					if ( ! empty( $discount ) ) {
						...
						if ( isset( $discount_type ) && $discount_type == 'amount' ) {
							...
							$base_price = ! empty( $base_price ) ? $base_price - $base_price * ( $discount / 100 ) : 0;
						}

						// $base_price       = ! empty( $base_price ) ? $base_price - $base_price * ( $discount / 100 ) : 0;
						if ( isset( $base_price ) && intval( $base_price ) < 0 ) {
							$base_price = 0;
						}
						...
						if ( $is_sale_schedule == 'on' ) {
							...
							echo '<span>' . __( 'Price', 'traveler' ) . ': ' . TravelHelper::format_money( $base_price ) . '</span><br>';
							if ( isset( $discount_type ) && $discount_type == 'amount' ) {
						...
						echo '<span>' . __( 'Price', 'traveler' ) . ': ' . TravelHelper::format_money( $base_price ) . '</span>';
					}
				} else {
					$discount          = get_post_meta( $post_ID, 'discount', true );
					$discount_type     = get_post_meta( $post_ID, 'discount_type', true );
					$price_adult       = ! empty( get_post_meta( $post_ID, 'adult_price', true ) ) ? (int)get_post_meta( $post_ID, 'adult_price', true ) : 0;
					$price_child       = ! empty( get_post_meta( $post_ID, 'child_price', true ) ) ? (int)get_post_meta( $post_ID, 'child_price', true ) : 0;
					$price_infant      = ! empty( get_post_meta( $post_ID, 'infant_price', true ) ) ? (int)get_post_meta( $post_ID, 'infant_price', true ) : 0;
					$str_discount_type = ( $discount_type == 'percent' ) ? '%' : '';
					if ( ! empty( $discount ) ) {
						$is_sale_schedule = get_post_meta( $post_ID, 'is_sale_schedule', true );
		...
		public static function data_layout() {
			return [
				'1' => esc_html__('Layout 1', 'traveler'),
				'2' => esc_html__('Layout 2', 'traveler'),
				'3' => esc_html__('Layout 3', 'traveler'),
				'4' => esc_html__('Layout 4', 'traveler'),
				'5' => esc_html__('Layout 5', 'traveler'),
				'6' => esc_html__('Layout 6', 'traveler'),
				'7' => esc_html__('Layout 7', 'traveler'),
				'8' => esc_html__('Layout modern 1', 'traveler'),
				'9' => esc_html__('Layout modern 2', 'traveler'),
			];
		}
				...
				if ( ! $updated ) {
					...
					if ( $list_tour->have_posts() ) {
						while ( $list_tour->have_posts() ) {
							$list_tour->the_post();
							$tour_id = get_the_ID();
							self::meta_update_min_price( $tour_id );
							self::_update_duplicate_data( $tour_id, [] );
						}
					}
					wp_reset_postdata();
