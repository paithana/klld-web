	...
	class STAdminCars extends STAdmin {
		...
		static function getInfoCarTranferPartner() {
			...
			$data          = [
				'price'          => 'not infomation',
				'item_equipment' => 'not infomation',
				'return_car'     => 'not infomation',
				'passenger'      => 'not infomation',
			];
			if ( $car_id <= 0 || get_post_type( $car_id ) != 'st_cars' ) {
				echo json_encode( $data );
				die();
				...
				if ( is_array( $item_equipment ) && count( $item_equipment ) ) {
					...
					foreach ( $item_equipment as $key => $val ) {
						...
						$cars_equipment_list_price = TravelHelper::convert_money( $val['extra_price'] );
						...
						for ( $i = 0; $i <= $max_item; $i++ ) {
							$html_option .= '<option value="' . $i . '">' . $i . '</option>';
						}

						$html .= '
                            <tr>
                                <td width="70%" style="vertical-align: middle;">
                                    <label for="field-' . $val['extra_name'] . '" class="ml20 mt5">
										' . $val['title'] . ' (' . TravelHelper::format_money( $val['extra_price'] ) . ')
									</label>
									...
									<input type="hidden" name="extra_price[extra_required][' . $val['extra_name'] . ']" value="' . $val['extra_required'] . '">
                                </td>
								<td>
									<select class="form-control app extra-service-select" name="extra_price[value][' . $val['extra_name'] . ']"
										id="field-' . $val['extra_name'] . '"
										data-extra-price="' . $val['extra_price'] . '"
									>
										' . $html_option . '
									</select>
								</td>
				...
				$get_transfer = $transfer->get_transfer( $car_id, $transfer_from, $transfer_to );
				...
				if ( ! empty( $return_car ) && ( $return_car === 'yes' ) ) {
					$html               = '
						<div class="input-group">
							<span><input type="radio" name="return_car"  value="yes"> Yes </span><br>
							<span><input type="radio" name="return_car" checked value="no"> No </span><br>
						</div>
					';
					$data['return_car'] = $html;
				}

				$price_type  = get_post_meta( $car_id, 'price_type', true );
				$number_pass = (int) get_post_meta( $car_id, 'num_passenger', true );
				if ( $price_type === 'passenger' && ! empty( $number_pass ) ) {
					$html = '<select name="passengers" class="form-control">';
					for ( $number_pas = 1; $number_pas <= $number_pass; $number_pas++ ) {
						$html .= '<option value="' . esc_attr( $number_pas ) . '">' . esc_html( $number_pas ) . '</option>';
					}
					$html .= '</select>';
		...
		static function getInfoCarPartner() {
			...
			if ( $car_id <= 0 || get_post_type( $car_id ) != 'st_cars' ) {
				...
				$price          = floatval( get_post_meta( $car_id, 'cars_price', true ) );
				$data['price']  = TravelHelper::format_money( $price ) . ' / ' . self::get_price_unit();
				$extra_price = get_post_meta( $car_id, 'extra_price', true );
				if ( is_array( $extra_price ) && count( $extra_price ) ) {
					ob_start();
					?>
						<ul class="extras">
				...
				$locations = TravelHelper::getListFullNameLocationCar($car_id,'st_cars');
					$result_location = array();
					$result_location[] = array(
						'id' => '',
						'text' => __('----Select Location----', 'traveler')
					);
					foreach($locations as $location){
						$result_location[] = array(
							'id' => $location->ID,
		...
		function _update_duplicate_data( $id, $data ) {
			...
			if ( get_post_type( $id ) == 'st_cars' ) {
				...
				if ( ! empty( $discount ) && ! empty( $sale_price ) ) {
			...
			$this->metabox[] = [
				...
				'fields'   => [
					...
					[
						...
						'id'           => 'passengers',
						'type'         => 'numeric-slider',
						'min_max_step' => '0,' . $value_baggage . ',1',
					...
					[
						...
						'std'          => '0',
						'condition'    => 'car_type:is(car_transfer)',
						'desc'         => __( 'No. Passengers for transfer', 'traveler' ),
					],
					[
						'label'     => __( 'Pricing for Car Rent', 'traveler' ),
						'id'        => 'cars_price',
						'type'      => 'text',
					...
					[
						...
						'type'      => 'list-item',
						'id'        => 'journey',
						'desc'      => __( 'Add the journey of the car transfer', 'traveler' ),
						'condition' => 'car_type:is(car_transfer)',
						'settings'  => [
							[
					...
					// ],
					[
						'label'     => __( 'Custom price for Car Rent', 'traveler' ),
						'id'        => 'is_custom_price',
						'std'       => 'off',
						'type'      => 'select',
						'choices'   => [
							[
								...
								'label' => __( 'Price by number of day|hour for Car Rent', 'traveler' ),
							],
							[
								'value' => 'price_by_date',
								'label' => __( 'Price by date for Car Rent', 'traveler' ),
							],
						],
						'condition' => 'car_type:is(normal)',
					],
		...
		function add_col_header( $defaults ) {
			$this->array_splice_assoc($defaults, 2, 0, [
				...
				'cars_type' => __( 'Car Type', 'traveler' ),
			]);
		...
		function add_col_content( $column_name, $post_ID ) {
			if ( $column_name == 'cars_type' ) {
				// show content of 'directors_name' column
				$car_type = get_post_meta( $post_ID, 'car_type', true );

				if ( $car_type === 'normal' ) {
					echo esc_html__( 'Normal', 'traveler' );
				} else {
					echo esc_html__( 'Car Transfer', 'traveler' );
				}
			}
			if ( $column_name == 'price' ) {
				...
				if ( $type == 'car_transfer' ) {
					...
					$price = ! empty( get_post_meta( $post_ID, 'cars_price', true ) ) ? get_post_meta( $post_ID, 'cars_price', true ) : 0;
				}
