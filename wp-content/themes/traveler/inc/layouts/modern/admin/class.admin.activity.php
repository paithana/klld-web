	...
	class STAdminActivity extends STAdmin {
		...
		static function _getInfoActivity() {
			...
			if ( get_post_type( $activity_id ) == 'st_activity' ) {
				...
				$disable_adult_name    = get_post_meta( $activity_id, 'disable_adult_name', true );
				$disable_children_name = get_post_meta( $activity_id, 'disable_children_name', true );
				$disable_infant_name   = get_post_meta( $activity_id, 'disable_infant_name', true );
				...
				if ( $disable_adult_name == 'off' || $disable_children_name == 'off' || $disable_infant_name == 'off' ) {
					$html_required_name .= '
                        <div class="guest_name_input d-none"
                            ...
                            data-hide-adult="' . get_post_meta( $activity_id, 'disable_adult_name', true ) . '"
                            data-hide-children="' . get_post_meta( $activity_id, 'disable_children_name', true ) . '"
                            >
		...
		function init_post_type() {
			...
			$args = [
				...
				'show_ui'            => true,
				'query_var'          => true,
				'rewrite'            => [ 'slug' => get_option( 'activity_permalink', 'st-activity' ) ],
		...
		function init_metabox() {
			...
			$layout_single   = apply_filters( 'st_layout_single_activity', $layout_single );
			$this->metabox[] = [
				'id'       => 'activity_metabox',
				...
				'fields'   => [
					...
					[
						...
						'id'    => 'enable_pickup',
						'type'  => 'on-off',
						'desc'  => __( 'Turn on/off Pick up field for activity booking. Applied ONLY for the default Modal Checkout', 'traveler' ),
						'std'   => 'off',
					],
					[
					...
					[
						...
						'id'           => 'max_people',
						'type'         => 'numeric-slider',
						'desc'         => __( 'Max number of people. Enter \'0\' for unlimited', 'traveler' ),
					...
					[
						...
						'id'        => 'child_price',
						'type'      => 'text',
						'desc'      => __( 'Price per child. Input number only. No decimal or thousand separator or any other character', 'traveler' ),
						'std'       => 0,
					...
					[
						...
						'id'        => 'infant_price',
						'type'      => 'text',
						'desc'      => __( 'Price per infant. Input number only. No decimal or thousand separator or any other character', 'traveler' ),
						'std'       => 0,
		...
		static function get_min_price( $post_id = null ) {
			...
			if ( $post_type == 'st_activity' ) {
				...
				if ( $off_adult != 'on' && ! empty( get_post_meta( $post_id, 'adult_price', true ) ) ) {
					$prices[] = get_post_meta( $post_id, 'adult_price', true );
				} else {
					$prices[] = 0;
				}
				if ( $off_child != 'on' && ! empty( get_post_meta( $post_id, 'child_price', true ) ) ) {
					$prices[] = get_post_meta( $post_id, 'child_price', true );
					if ( $off_infant != 'on' && ! empty( get_post_meta( $post_id, 'infant_price', true ) ) ) {
		...
		function _save_booking( $order_id ) {
			...
			if ( $this->_check_validate() ) {
				...
				if ( TravelHelper::checkTableDuplicate( 'st_activity' ) ) {
					...
					$table_activity_availability = $wpdb->prefix . 'st_activity_availability';
					...
					if ( $status_order != 'complete' && $_POST['status'] == 'complete' ) {
						AvailabilityHelper::syncAvailabilityAfterCompleted( $order_id );
					} elseif ( $status_order == 'complete' && $_POST[ 'status' ] != 'complete' ) {
                        $sql = $wpdb->prepare( "UPDATE {$table_activity_availability} SET number_booked = GREATEST( 0, IFNULL(number_booked, 0) - %d ) WHERE post_id = %d AND check_in = %s", $booked, $post_id, $check_in_timestamp );
                        $wpdb->query($sql);
                    }
			...
			if ( $column_name == 'price' ) {
				...
				if ( ! empty( $discount ) ) {
					...
					if ( isset( $discount_type ) && $discount_type == 'amount' ) {
						$sale_adult = ! empty( $price_adult ) ? $price_adult - $discount : 0;
						$sale_child = ! empty( $price_child ) ? $price_child - $discount : 0;
					}
		...
		public static function data_layout() {
			return [
				...
				'4' => esc_html__('Layout modern 1', 'traveler'),
				'5' => esc_html__('Layout modern 2', 'traveler'),
			];
		}
		...
		public static function __cronjob_update_min_price( $offset, $limit = 2 ) {
			global $wpdb;
			$list_activity = new WP_Query([
				'posts_per_page' => $limit,
				'post_type'      => 'st_activity',
				'offset'         => $offset,
			]);

			if ( $list_activity->have_posts() ) {
				while ( $list_activity->have_posts() ) {
					$list_activity->the_post();
					$activity_id = get_the_ID();
					self::meta_update_min_price( $activity_id );
					self::_update_duplicate_data( $activity_id, [] );
				}
			}
