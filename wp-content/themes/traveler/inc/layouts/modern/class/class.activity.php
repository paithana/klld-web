    ...
    if ( !class_exists( 'STActivity' ) ) {
        ...
        class STActivity extends TravelerObject
        {
            ...
            function init()
            {
				...
				// Half Map
				add_action( 'wp_ajax_st_filter_activity_map', [ $this, '__getMapFilterAjax' ] );
				add_action( 'wp_ajax_nopriv_st_filter_activity_map', [ $this, '__getMapFilterAjax' ] );
            }
				...
				if ( $query->have_posts() ) {
					while ( $query->have_posts() ) {
						...
						if ( ! empty( $map_lat ) && ! empty( $map_lng ) ) {
							...
							if ( $isNew ) {
								$data_map[$stt]['content_html'] = preg_replace( '/^\s+|\n|\r|\s+$/m', '', stt_elementorv2()->loadView( 'services/activity/components/map-popup' ) );
								$data_map[$stt]['content_adv_html'] = preg_replace( '/^\s+|\n|\r|\s+$/m', '', stt_elementorv2()->loadView( 'services/activity/loop-adv/activity', false, ['post_type' => $post_type_name->label] ) );
							} else {
								$data_map[$stt]['content_html'] = preg_replace( '/^\s+|\n|\r|\s+$/m', '', st()->load_template( 'layouts/modern/activity/elements/map-popup' ) );
                    ...
                    while ($query->have_posts()) {
                        ...
                        if(function_exists('check_using_elementor') && check_using_elementor()){
                            ...
                            if ($version == 'elementorv2') {
                                ...
                                if($layoutRequest == 4){
                                    ...
                                    if(!empty($st_item_row)){
                                        ...
                                        $classes = ( $st_style == 'grid' ) ? 'col-lg-3 col-md-6 col-sm-6 col-12 item-service' : 'col-lg-12 col-md-12 col-12 item-service';
                                    }
                                }
							...
							if ( STInput::request( 'format' ) == 'halfmap' ) {
								echo st()->load_template( 'layouts/modern/activity/elements/loop/halfmap' );
							} else {
            ...
            static function _get_order_by_query($orderby, $wp_query) {
                ...
                if ($check = STInput::get('orderby')) {
                    ...
                    if(empty($wp_query->query['orderby'])){
                        $is_featured = st()->get_option('is_featured_search_activity', 'off');
                        if (!empty($is_featured) and $is_featured == 'on') {
							$orderby = 'tb_st_activity.is_featured desc, ' . $wpdb->posts . '.post_date desc';
                        } else {
            ...
            function do_add_to_cart()
            {
				...
				$format_check_out = date("Y-m-d", strtotime($check_out));
				$starttime_data   = AvailabilityHelper::_get_starttime_activity_by_date($item_id, strtotime($format_check_in), strtotime($format_check_out));
                ...
                $data['discount_type'] = get_post_meta( $post_origin, 'discount_type', true );
                if ( $pass_validate ) {
                    $data[ 'duration' ] = get_post_meta( $item_id, 'duration', true );
                    if ( $pass_validate ) {
            ...
            static function get_price_html( $post_id = false, $get = false, $st_mid = '', $class = '', $hide_title = true )
            {
				...
				$post_type        = 'st_activity_availability';
				$current_calendar = TravelHelper::get_current_available_calendar(TravelHelper::post_origin(get_the_ID(), 'st_activity'));
				...
				$where = $wpdb->prepare(
					"WHERE 1=1
					AND ({$wpdb->prefix}st_activity_availability.post_id = %d AND `check_in` >= %d)
					AND status = 'available'",
					$post_id, $check_in
				);
				$sql      = "SELECT
						min(CAST(`adult_price` as DECIMAL(15,6))) as min
					FROM
					...
					switch ( $discount_type ) {
						...
						default:
							if ( isset( $discount ) && ( $discount > 0 ) ) {
								$price_old = $price_new - ( $price_new / 100 ) * $discount;
							} else {
								$price_old = $price_new;
							}
							break;
                ...
                if($class=='sale-top'){
                    ...
                    $html .= '<span class="text-lg lh1em item "> ' . TravelHelper::format_money( $price_old ). "</span>";
                    $html .= '</span>';
                } else {
                    if ( !$hide_title and $price_new > 0 ) {
                        $html .= __( "From", 'traveler' );
                    ...
                    $price_old = TravelHelper::format_money( $price_old );
                    $html .= '<span class="text-lg lh1em item "> ' . $price_old . "</span>";
                }
