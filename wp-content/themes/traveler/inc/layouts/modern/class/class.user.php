    ...
    if ( !class_exists( 'STUser_f' ) ) {
        class STUser_f extends TravelerObject
        {
            ...
            function init()
            {
				...
				add_action( 'woocommerce_after_order_itemmeta', [ $this, 'st_order_itemmeta_refund' ], 10, 3 );
			...
			public function st_order_itemmeta_refund( $item_id, $item, $product ){
				// Only "line" items and backend order pages
				if( ! ( is_admin() && $item->is_type( 'line_item' ) ) ) return;

				$cancel_data = $item->get_meta( '_st_cancel_data' );
				...
				echo '
					...
					<div class="alert alert-info mt20" role="alert">
						<div class=" clearfix">
							<strong>Acount Name: </strong><span class=""><em>'. $cancel_data['your_bank']['account_name'] .'</em></span>
						</div>
						<div class="mt10 clearfix">
							<strong>Acount Number: </strong><span class=""><em>'. $cancel_data['your_bank']['account_number'] .'</em></span>
						</div>
						<div class="mt10 clearfix">
							<strong>Bank Name: </strong><span class=""><em>'. $cancel_data['your_bank']['bank_name'] .'</em></span>
						</div>
						<div class="mt10 clearfix">
							<strong>Swift Code: </strong><span class=""><em>'. $cancel_data['your_bank']['swift_code'] .'</em></span>
						</div>
					</div>
				';
			}
            ...
            public function __stPartnerCreateServiceFlight(){
				...
				if ( ! current_user_can( 'edit_post', STInput::request('post_id') ) && ! empty( STInput::request('post_id') ) ) {
					echo json_encode(array(
						'status' => false,
						'err' => __("You are not allowed to create flight", 'traveler')
					));
					die;
				}

				if ( ! isset( $_POST['security'] ) || !wp_verify_nonce( $_POST['security'], 'partner_create_service_flight') ) {
					echo json_encode( [
						'status'  => 'error',
						'message' => __( 'You are not allowed', 'traveler' )
					] );
					die;
				}
            ...
            public function __stPartnerCreateServiceRentalRoom(){
				if ( ! current_user_can( 'edit_post', STInput::request('post_id') ) && ! empty( STInput::request('post_id') ) ) {
					echo json_encode(array(
						'status' => false,
						'err' => __("You are not allowed to create rental room", 'traveler')
					));
					die;
				}

				if ( ! isset( $_POST['security'] ) || !wp_verify_nonce( $_POST['security'], 'partner_create_service_rentalroom') ) {
					echo json_encode( [
						'status'  => 'error',
						'message' => __( 'You are not allowed', 'traveler' )
					] );
                ...
                switch ($step_name){
                    case 'basic_info':
                        ...
                        if($valid){
                            ...
                            $err = $this->stSetErrorMessage(array('st_title', 'st_content', 'room_parent'));
                            echo json_encode(array(
                                'status' => false,
                                'err' => $err
                            ));
            ...
            public function __stPartnerCreateServiceRental(){
				if ( ! current_user_can( 'edit_post', STInput::request('post_id') ) && ! empty( STInput::request('post_id') ) ) {
					echo json_encode(array(
						'status' => false,
						'err' => __("You are not allowed to create rental", 'traveler')
					));
					die;
				}

				if ( ! wp_verify_nonce( $_REQUEST[ 'st_update_rental' ], 'user_setting' ) ) {
					echo json_encode(array(
						'status' => false,
                ...
                switch ($step_name){
                    case 'basic_info':
                        ...
                        if($valid){
                            ...
                            if (!empty($post_id)) {
                                ...
                                wp_update_post($my_post);
                                update_post_meta( $post_id, 'rental_number', (int)STInput::request( 'rental_number' ) );
                                update_post_meta( $post_id, 'rental_max_adult', (int)STInput::request( 'rental_max_adult' ) );
                                update_post_meta( $post_id, 'rental_max_children', (int)STInput::request( 'rental_max_children' ) );
                                ...
                                update_post_meta( $post_id, 'rental_single_layout_v2', (int)STInput::request( 'rental_single_layout_v2' ) );
								...
								do_action( 'st_partner_rental', $post_id );

								$this->_update_badges( $post_id, $_REQUEST[ 'title' ], $_REQUEST[ 'content' ], $_REQUEST[ 'icon' ] );
								$this->_update_upsell( $post_id, STInput::request( 'type-service' ), STInput::request( 'list-item' ) );
                            }
                            echo json_encode(array(
                                'status' => true,
                                'next_step' => 2,
                            ...
                            $arr_err = array('st_title', 'st_content', 'is_featured', 'rental_number', 'rental_max_adult', 'rental_max_children', 'rental_bed', 'rental_bath', 'rental_size');
                    ...
                    case 'info':
                        ...
                        if(!empty($post_id)){
                            ...
                            if($valid){
								...
								do_action( 'st_partner_rental_tab_info', $post_id );
                    ...
                    case 'prices':
                        ...
                        if(!empty($post_id)) {
                            ...
                            if($valid) {
                                    ...
                                    foreach ( $extra[ 'title' ] as $key => $val ) {
                                        if ( !empty( $val ) ) {
                                            $list_extras[ $key ] = [
                                                ...
                                                'extra_required' => isset($extra['extra_required'][$key]) ? $extra['extra_required'][$key] : ''
                                            ];
                                        }
                                    }
                                    ...
                                    foreach ( $discount_by_day[ 'title' ] as $key => $val ) {
                                        if ( !empty( $val ) ) {
                                            $list_discount_by_day[ $key ] = [
                                                'title'      => $val,
                                                'number_day' => !empty( $discount_by_day[ 'number_day' ][ $key ] ) ? $discount_by_day[ 'number_day' ][ $key ] : '',
                                                'number_day_to'   => !empty( $discount_by_day[ 'number_day_to' ][ $key ] ) ? $discount_by_day[ 'number_day_to' ][ $key ] : '',
                                                'discount'   => !empty( $discount_by_day[ 'discount' ][ $key ] ) ? $discount_by_day[ 'discount' ][ $key ] : '',
                                            ];
                                        }
                                    }
                                ...
                                update_post_meta( $post_id, 'extra_price_unit', STInput::request( 'extra_price_unit' ) );
								...
								do_action( 'st_partner_rental_price', $post_id );
                                echo json_encode(array(
                                    'status' => true,
                                    'next_step' => 5,
                                    'next_step_name' => $step_name,
                    ...
                    case 'availability':
                        $post_id = STInput::post('post_id');
                        if(!empty($post_id)) {
							do_action( 'st_partner_rental_tab_availability', $post_id );
                            if($step != 'final') {
                                echo json_encode(array(
                                    'status' => true,
                                    'next_step' => 8,
            ...
            private function stRentalValidate($step = 1){
                ...
                switch ($step){
                    case 1:
                        ...
                        $validator->set_rules('rental_size', __("Size of Area", 'traveler'), 'required');
						do_action( 'st_rule_rental', $validator );
                        if(isset($_REQUEST['st_rental_external_booking']) && $_REQUEST['st_rental_external_booking'] == 'on'){
                    ...
                    case 2:
                        ...
                        $validator->set_rules('agent_website', __("Website", 'traveler'), 'valid_url');
                    ...
                    case 5:
						...
						$validator->set_rules('multi_location', __("Location", 'traveler'), 'required');
                        $location = $_REQUEST['multi_location'];
            ...
            public function st_partner_car_info($fields){
                ...
                $arr_temp =  array(
                    ...
                    array(
                        'type' => 'text',
                        'label' => __('Minimum number of days to book', 'traveler'),
            ...
            public function __stPartnerCreateServiceCar(){
				if ( ! current_user_can( 'edit_post', STInput::request('post_id') ) && ! empty( STInput::request('post_id') ) ) {
					echo json_encode(array(
						'status' => false,
						'err' => __("You are not allowed to create car", 'traveler')
					));
					die;
				}

				if ( ! isset( $_POST['security'] ) || !wp_verify_nonce( $_POST['security'], 'partner_create_service_car') ) {
					echo json_encode( [
						'status'  => 'error',
						'message' => __( 'You are not allowed', 'traveler' )
					] );
                ...
                switch ($step_name){
                    case 'basic_info':
                        ...
                        if($valid){
                            ...
                            $err = $this->stSetErrorMessage(array('st_title', 'is_featured','st_content'));
                            echo json_encode(array(
                                'status' => false,
                                'err' => $err
                            ));
            ...
            private function stCarValidate($step = 1){
                ...
                switch ($step){
                    ...
                    case 3:
                        ...
                        break;
                    case 4:
						$validator->set_rules('car_type', __("Car Types", 'traveler'), 'required');
            ...
            public function __stPartnerCreateServiceActivity(){
				if ( ! current_user_can( 'edit_post', STInput::request('post_id') ) && ! empty( STInput::request('post_id') ) ) {
					echo json_encode(array(
						'status' => false,
						'err' => __("You are not allowed to create activity", 'traveler')
					));
					die;
				}

				if ( ! isset( $_POST['security'] ) || !wp_verify_nonce( $_POST['security'], 'partner_create_service_activity') ) {
					echo json_encode( [
						'status'  => 'error',
						'message' => __( 'You are not allowed', 'traveler' )
					] );
                ...
                switch ($step_name){
                    case 'basic_info':
                        ...
                        if($valid){
                            ...
                            $err = $this->stSetErrorMessage(array('st_title', 'st_content', 'is_featured', 'contact_email', 'contact_web', 'video'));
                            echo json_encode(array(
                                'status' => false,
                                'err' => $err
                            ));
                    ...
                    case 'info':
                        ...
                        if(!empty($post_id)){
                            ...
                            if($valid){
								...
								update_post_meta($post_id, 'enable_pickup', STInput::request('enable_pickup'));
                    ...
                    case 'prices':
                        ...
                        if(!empty($post_id)) {
                            ...
                            if($valid) {
								...
								do_action( 'st_partner_activity_tab_price', $post_id );
                                echo json_encode(array(
                                    'status' => true,
                                    'next_step' => 5,
                                    'next_step_name' => $step_name,
                ...
                switch ($step){
                    ...
                    case 4:
                        ...
                        if(isset($_REQUEST['st_allow_cancel']) && $_REQUEST['st_allow_cancel'] == 'on'){
                            ...
                            $validator->set_rules('st_cancel_percent', __("Cancellation Fee", 'traveler'), 'required');
                        }
                        // if(isset($_REQUEST['best-price-guarantee']) && $_REQUEST['best-price-guarantee'] == 'on'){
            ...
            public function st_partner_activity_info($fields){
                ...
                //Layout activity single
                $array_activity_elementor=[];
                $layout_activity_single = [
                    [
                        'value' => '1',
				...
				$booking_option = apply_filters( 'st_booking_option_activity', $booking_option );

                if(check_using_elementor()){
                    $array_activity_elementor = [
                        [
                            'value' => '4',
                            'label' => esc_html__('Layout modern 1', 'traveler'),
                            'src' => get_template_directory_uri() . '/v2/images/layouts/tour_detail_layout_8.jpg',
                        ],
                        [
                            'value' => '5',
                            'label' => esc_html__('Layout modern 2', 'traveler'),
                            'src' => get_template_directory_uri() . '/v2/images/layouts/tour_detail_layout_9.jpg',
                        ],
                    ];
                    $layout_activity_single = array_merge($layout_activity_single,$array_activity_elementor);
                }
                $layout_activity_single = apply_filters('st_layout_single_activity',$layout_activity_single);

                //End layout activity single
                $new_arr = array(
                    array(
                        'type' => 'select',
                    ...
                    array(
                        ...
                        'col' => '12',
                        'plh' => '',
                        'choices'   => $layout_activity_single,
                    ),
                    array(
                        'type' => 'select',
                        'label' => __('External booking', 'traveler'),
					...
					array(
						'type' => 'select',
						'label' => __('Turn on/off Pick up', 'traveler'),
						'name' => 'enable_pickup',
						'col' => '3',
						'plh' => '',
						'required' => false,
            ...
            public function __stPartnerCreateServiceTour(){
				if ( ! current_user_can( 'edit_post', STInput::request('post_id') ) && ! empty( STInput::request('post_id') ) ) {
					echo json_encode(array(
						'status' => false,
						'err' => __("You are not allowed to create tour", 'traveler')
					));
					die;
				}

				if ( ! isset( $_POST['security'] ) || !wp_verify_nonce( $_POST['security'], 'partner_create_service_tour') ) {
					echo json_encode( [
						'status'  => 'error',
						'message' => __( 'You are not allowed', 'traveler' )
					] );
                    ...
                    case 'info':
                        ...
                        if(!empty($post_id)){
                            ...
                            if($valid){
								...
								do_action( 'st_partner_tour_tab_info', $post_id );
            ...
            public function st_partner_tour_info($fields){
                ...
                if(!check_using_elementor()){
                    ...
                    $layout_tour_single = array_merge($layout_tour_single,$array_modern);
                }
                if(check_using_elementor()){
                    $array_layout_tour_elementor = [
                        [
                    ...
                    $layout_tour_single = array_merge($layout_tour_single,$array_layout_tour_elementor);
                }
                $layout_tour_single = apply_filters('st_layout_single_tour',$layout_tour_single);
                //End tour layout
                $new_arr = array(
                    array(
                        'type' => 'select',
            ...
            public function __stPartnerCreateServiceRoom(){
				if ( ! current_user_can( 'edit_post', STInput::request('post_id') ) && ! empty( STInput::request('post_id') ) ) {
					echo json_encode(array(
						'status' => false,
						'err' => __("You are not allowed to create room", 'traveler')
					));
					die;
				}

				if ( ! isset( $_POST['security'] ) || !wp_verify_nonce( $_POST['security'], 'partner_create_service_room') ) {
					echo json_encode( [
						'status'  => 'error',
						'message' => __( 'You are not allowed', 'traveler' )
					] );
                    ...
                    case 'basic_info':
                        ...
                        if($valid){
                            ...
                            if (!empty($post_id)) {
                                ...
                                if(!empty(STInput::request('room_parent'))){
                                    update_post_meta($post_id, 'room_parent', STInput::request('room_parent'));
                                }
                            ...
                            $arr_field = array('st_title', 'st_content', 'number_room', 'adult_number', 'children_number', 'bed_number', 'room_footage');
                    ...
                    case 'prices':
                        ...
                        if(!empty($post_id)) {
                            ...
                            if($valid) {
                                    ...
                                    foreach ( $discount_by_day[ 'title' ] as $key => $val ) {
                                        if ( !empty( $val ) ) {
                                            $list_discount_by_day[ $key ] = [
                                                ...
                                                'number_day_to'   => isset( $discount_by_day[ 'number_day_to' ][ $key ] ) ? $discount_by_day[ 'number_day_to' ][ $key ] : '',
                                ...
                                update_post_meta( $post_id, 'disable_infant_name', STInput::request( 'disable_infant_name' ) );
                                update_post_meta( $post_id, 'is_sale_schedule', STInput::request( 'is_sale_schedule' ) );
								do_action( 'st_partner_room', $post_id );
            ...
            public function __stPartnerCreateService(){
				if ( ! current_user_can( 'edit_post', STInput::request('post_id') ) && ! empty( STInput::request('post_id') ) ) {
					echo json_encode(array(
						'status' => false,
						'err' => __("You are not allowed to create hotel", 'traveler')
					));
					die;
				}

				if ( ! isset( $_POST['security'] ) || !wp_verify_nonce( $_POST['security'], 'partner_create_service') ) {
					echo json_encode( [
						'status'  => 'error',
						'message' => __( 'You are not allowed', 'traveler' )
					] );
                    ...
                    case 'basic_info':
                        ...
                        if($valid){
                            ...
                            if (!empty($post_id)) {
								...
								update_post_meta($post_id, 'st_hotel_external_booking', STInput::request('st_hotel_external_booking'));
                                update_post_meta($post_id, 'st_hotel_external_booking_link', STInput::request('st_hotel_external_booking_link'));
                            ...
                            $err_field = array('st_title', 'st_content', 'hotel_star','is_featured', 'id_logo', 'hotel_booking_period');
			...
			public function _st_partner_approve_booking()
            {
                ...
                if ( !empty($order_id) ) {
                    ...
                    $data_order = self::get_history_bookings_by_id( $order_id );
                    if ( !empty( $data_order ) ) {
                        $status = 'pending';
                        if ( $data_order->type == "normal_booking" ) {
                            $status = get_post_meta( $order_id, 'status', true );
                            $res = update_post_meta( $order_id, 'status', 'complete' );
                            if ( TravelHelper::checkTableDuplicate( 'st_tours' ) ||  TravelHelper::checkTableDuplicate( 'st_activity' )
                            ||  TravelHelper::checkTableDuplicate( 'st_cars' ) ||  TravelHelper::checkTableDuplicate( 'st_hotel' )
                            ) {
                                global $wpdb;
                                $table = $wpdb->prefix . 'st_order_item_meta';
                                $data  = [
                                    'status' => 'complete',
                                    'message' => __( 'complete', 'traveler' )
                                ];
                                $where = [
            ...
            public function _send_partner_notice_departure_date( $order_id )
            {
                ...
                if ( $order_id ) {
                    $message = "";
                    $id_page_email_for_customer = st()->get_option('email_for_customer_out_of_depature_date', '');
                    $email_content = !empty(get_post($id_page_email_for_customer)) ? wp_kses_post(get_post($id_page_email_for_customer)->post_content) : "";
                    $message  .= TravelHelper::_get_template_email( $message, $email_content );
                    $data     = STUser_f::get_history_bookings_by_wc_order_id( $st_order_id );
            ...
            function _st_get_info_booking_history()
            {
                ...
                $user_book    = 0;
                global $wpdb;
                $sql        = $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}st_order_item_meta WHERE (order_item_id = %d or wc_order_id = %d) AND  st_booking_id = %d", $order_id, $order_id, $service_id ) ;
                ...
                if ( STInput::post( 'action', '' ) == 'reset_password' &&
                    ...
                    if ( $update_user ) {
                        ...
                        if ( $mail ) {
                            ...
                            STTemplate::set_message( __( 'Oops something went wrong updating your account.', 'traveler' ), 'danger' );
                            return false;
                        }
                    } else {
                        STTemplate::set_message( __( 'Oops something went wrong updating your account.', 'traveler' ), 'danger' );
            ...
            {
                $where .= " AND od.`status` = 'cancelling'
            ...
            public function st_check_complete_refund()
            {
                ...
                $id_item        = (int)get_post_meta( $order_id, 'item_id', true );
                $post_author_id = get_post_field( 'post_author', $id_item );
                ...
                if ( TravelHelper::st_compare_encrypt( $order_id, $order_encrypt ) ) {
                    ...
                    if ( $cancel_data ) {
						...
						update_post_meta( $order_id, 'status', 'canceled' );
                        global $wpdb;
                        $query = "UPDATE {$wpdb->prefix}st_order_item_meta set `status`='canceled', cancel_refund_status='complete' where order_item_id={$order_id}";
                        $wpdb->query( $query );
            ...
            public function st_get_cancel_booking_step_1()
            {
                ...
                if ( TravelHelper::st_compare_encrypt( $order_id, $order_encrypt ) ) {
					...
					if($st_is_woocommerce_checkout){
						$rs          = st_get_order_by_order_item_id($order_id);
						$item_id     = (int)$rs['st_booking_id'];
						$post_type   = $rs['st_booking_post_type'];
						$status      = $rs['status'];
						$total_price = (float)$rs['total_order'];
						$percent     = (int)get_post_meta( $item_id, 'st_cancel_percent', true );
						...
						$step = 'next-to-step-2';
						if ( $status != 'wc-completed' || ( $percent == 100 && $refunded == 0 ) ) {
							$step = 'next-to-step-3';
						}
					}
                    echo json_encode( [
            ...
            public function st_get_cancel_booking_step_3()
            {
                ...
                if ( TravelHelper::st_compare_encrypt( $order_id, $order_encrypt ) ) {
					...
					if($st_is_woocommerce_checkout){
						...
						if ( !in_array($status, ['wc-completed', 'completed']) ) {
							$refunded             = 0;
						...
						$query = "UPDATE {$wpdb->prefix}st_order_item_meta set `status`='wc-cancelled', cancel_refund='{$refunded}' , cancel_refund_status='{$cancel_refund_status}' where order_item_id={$order_id}";
						$wpdb->query( $query );

						$wc_order_id = !empty($rs["wc_order_id"]) ? $rs["wc_order_id"] : "";
						if(!empty($wc_order_id)){
							$order = new WC_Order($wc_order_id);
							...
							// $order->update_meta_data( 'cancel_data', 'true' );
							wc_add_order_item_meta( $order_id,'_st_cancel_data', $cancel_data );
						}

						$message = st()->load_template( 'user/cancel-booking/success', 'none' );
                    ...
                    if ( empty( $select_account ) ) {
                        ...
                        if ( $cancel ) {
                            //Update number_booked
                            AvailabilityHelper::syncAvailabilityAfterCanceled( $order_id );
                            $query = "UPDATE {$wpdb->prefix}st_order_item_meta set `status`='cancelling', cancel_refund='{$refunded}' , cancel_refund_status='{$cancel_refund_status}' where order_item_id={$order_id}";
                            $wpdb->query( $query );
                            $st_is_woocommerce_checkout = apply_filters( 'st_is_woocommerce_checkout', false );
                            if($st_is_woocommerce_checkout){
                                $wc_order_id = !empty(st_get_order_by_order_item_id($order_id)["wc_order_id"]) ? st_get_order_by_order_item_id($order_id)["wc_order_id"] : "";
                                if(!empty($wc_order_id)){
                                    $order = new WC_Order($wc_order_id);
                                    $order->update_status('wc-cancelled');
                                    update_post_meta( $wc_order_id, 'status', 'Cancelled' );
                                }
                            ...
                            if ( $status == 'incomplete' || $status == 'pending') {
                                ...
                                if ( $enable_partner_email_cancel == 'on' ) {
                                    if ( $user_role === 'partner' ) {
                                        $this->_send_email_refund_for_partner( $order_id, $user_email, 'has-refund' );
                                    }
                                }
                            }
                            echo json_encode( [
                    ...
                    if ( $select_account == 'your_bank' ) {
                        ...
                        if ( $cancel ) {
                            ...
                            $wpdb->query( $query );
                            update_post_meta( $order_id, 'cancel_data', $cancel_data );
							update_post_meta( $order_id, 'status', 'cancelling' );
                            unset( $_SESSION[ 'cancel_data' ] );
							session_write_close();
            ...
            public function _send_email_refund_for_partner( $order_id = '', $to = '', $type = false )
            {
                if ( $type === "success" ) {
                    ...
                    $id_page_email_cancel_booking_success_for_partner = st()->get_option( 'email_cancel_booking_success_for_partner', '' );
                    $template = !empty(get_post($id_page_email_cancel_booking_success_for_partner)) ? wp_kses_post(get_post($id_page_email_cancel_booking_success_for_partner)->post_content) : "";
                } else {
                    $subject  = __( 'You have a request for cancel booking.', 'traveler' );
                    $id_page_email_has_refund_for_partner = st()->get_option( 'email_has_refund_for_partner', '' );
                    $template = !empty(get_post($id_page_email_has_refund_for_partner)) ? wp_kses_post(get_post($id_page_email_has_refund_for_partner)->post_content) : "";
                }
            ...
            public function _send_email_refund( $order_id = '', $type = 'success' )
            {
                ...
                if ( $type == 'success' ) {
                    $to       = get_post_meta( $order_id, 'st_email', true );
                    $subject  = __( 'Your request cancel booking is completed.', 'traveler' );
                    $id_page_email_cancel_booking_success = st()->get_option( 'email_cancel_booking_success', '' );
                    $template = !empty(get_post($id_page_email_cancel_booking_success)) ? wp_kses_post(get_post($id_page_email_cancel_booking_success)->post_content) : "";
                } elseif ( $type == 'has-refund' ) {
                    $to      = st()->get_option( 'email_admin_address', '' );
                    $subject = __( 'You have a request for cancel booking.', 'traveler' );
                    $id_page_email_has_refund = st()->get_option( 'email_has_refund', '' );
                    $template = !empty(get_post($id_page_email_has_refund)) ? wp_kses_post(get_post($id_page_email_has_refund)->post_content) : "";
                    ...
                    if (!is_user_logged_in()) {
                        ...
                        if (!empty($page_login)) {
                            ...
                            if(!empty($link)){
                                ...
                                $location = add_query_arg([' ' => urlencode($url_redirect)], $link);
            ...
            {
                $theme_layout = STInput::request('st_theme_style', 'modern');
            ...
            static function _update_info_user( $register_user )
            {
                ...
                if($theme_layout == 'classic'){
                    ...
                    switch ($register_as) {
                        case "normal":
                            STUser::_send_admin_new_register_user($register_user);
							STUser::_send_customer_register_normal($register_user);
                            $responsive = [
                                'status' => 1,
            ...
            function st_add_wishlist_func()
            {
                ...
                $tmp_data = [];
                $icon_checked = 'fa fa-heart';
                $icon_not_checked = 'fa fa-heart-o';
                if(check_using_elementor()){
                    $icon_checked = 'fas fa-heart';
                    $icon_not_checked = 'far fa-heart';
                }
                if ( $data_list != '' and is_array( $data_list ) ) {
                    ...
                    if ( $check == true ) {
                        ...
                        echo json_encode( [
                            'status' => 'true',
                            'msg'    => 'ID :' . $data_id,
                            'icon'   => '<i class="'.esc_html($icon_checked).'"></i>',
                        ...
                        echo json_encode( [
                            'status' => 'true',
                            'msg'    => 'ID :' . $data_id,
                            'icon'   => '<i class="'.esc_html($icon_not_checked).'"></i>',
            ...
            {
				if ( ! current_user_can( 'edit_post', $_REQUEST[ 'data_id' ] ) && ! empty( $_REQUEST[ 'data_id' ] ) ) {
					echo json_encode(array(
						'status' => false,
						'content' => __("You are not allowed to delete", 'traveler')
					));
					die;
				}

				if ( ! isset( $_POST['security'] ) || !wp_verify_nonce( $_POST['security'], 'st_frontend_security') ) {
					echo json_encode( [
						'status'  => 'error',
						'content' => __( 'You are not allowed', 'traveler' )
					] );
					die;
				}
            ...
            static function _get_order_statuses()
            {
                ...
                if ( $st_is_woocommerce_checkout ) {
                    ...
                    $order_statuses = [
                        ...
                        'cancelling' => __( 'Cancelling', 'traveler' ),
                    ];
                }
            ...
            function get_book_history( $status = '' )
            {
                if ( st()->get_option('use_woocommerce_for_booking') == 'on' ) {
					...
					if ( in_array( $status, [ 'refunded', 'wc-refunded' ] )  ) {
						$where .= " AND status = 'refunded' OR status = 'wc-refunded' ";
					} elseif ( in_array( $status, [ 'checkout-draft', 'wc-checkout-draft' ] ) ) {
						$where .= " AND status = 'checkout-draft' OR status = 'wc-checkout-draft' ";
					} elseif ( !empty( $status ) ) {
                ...
                if ( !empty( $pageposts ) ) {
                    foreach ( $pageposts as $key => $value ) {
                        ...
                        if ( TravelerObject::check_cancel_able( $value->order_item_id )  ) {
                            ...
                            if(!st_check_is_checkout_woocomerce(false)){
                                ...
                                //If turn on multi item cart woo then not show cancel
                                $multi_item_in_cart = st()->get_option( 'multi_item_in_cart', 'off' );
                                if($multi_item_in_cart === 'off'){
                                    $action .= '<a  data-toggle="modal" data-target="#cancel-booking-modal" class="btn btn-xs btn-primary mt5 confirm-cancel-booking" href="javascript: void(0);" data-order_id="' . $value->order_item_id . '" data-order_encrypt="' . TravelHelper::st_encrypt( $value->order_item_id ) . '"><i class="fa fa fa-ban"></i><span class="hidden-xs">' . __( 'Cancel Booking', 'traveler' ) . '</span></a>';
                                }
                            }
                    ...
                    if ( $check == true ) {
                        if($style == 2){
                            return [
                                ...
                                'icon'           => '<i class="'.esc_html($icon_checked).' added"></i>',
                                'status' => true
                            ];
                        }
                        return [
        ...
        */
                ...
                ";
                    ...
                    if ( !empty( $data_items ) ) {
                        foreach ( $data_items as $k => $v ) {
                            ...
                            if ( $v->type == 'woocommerce' && ( st()->get_option('use_woocommerce_for_booking') == 'on' ) ) {
            ...
            static function _st_load_month_by_year_partner()
            {
                ...
                $sql         = "SELECT * FROM  " . $wpdb->prefix . "st_order_item_meta
                        ...
                        AND partner_id = " . $user_id . "
                        AND (status = 'complete' OR status = 'wc-completed' OR status = 'wc-pending' OR status='wc-processing'  OR ( status = 'canceled' AND CAST( cancel_refund as UNSIGNED ) > 0 AND cancel_refund_status IN ('complete', 'pending','wc-completed', 'wc-pending')) )
                        OR ( status = 'wc-cancelled' AND CAST( cancel_refund as UNSIGNED ) > 0 AND cancel_refund_status IN ('wc-completed', 'wc-pending'))
                        AND st_booking_post_type IN ('" . $post_type . "')
                        AND st_booking_post_type not in {$list_not_in}
                        GROUP BY wc_order_id
            ...
            if($number >= 0){
                ...
                if ( st()->get_option('use_woocommerce_for_booking') == 'on' ) {
                    ...
                    WHERE 1=1
                    AND partner_id = " . $user_id . "
                    AND (status = 'complete' OR status = 'wc-completed' OR ( status = 'canceled' AND CAST( cancel_refund as UNSIGNED ) > 0 AND cancel_refund_status IN ('complete', 'pending' , 'wc-completed', 'wc-pending')) )
                    OR ( status = 'wc-cancelled' AND CAST( cancel_refund as UNSIGNED ) > 0 AND cancel_refund_status IN ('wc-completed', 'wc-pending'))
                    AND st_booking_post_type not in {$list_not_in}
                    GROUP BY order_item_id";
                } else {
            ...
            static function _cancel_booking( $order_id )
            {
                ...
                if ( $check_cancel_able ) {
                    ...
                    $user_id       = get_current_user_id();
                    $order_item_id = $order_id;
                    $check_order = $wpdb->get_row( "SELECT * FROM {$wpdb->prefix}st_order_item_meta where user_id={$user_id} and  order_item_id = {$order_item_id} and `status`!='canceled' and `status`!='wc-cancelled' LIMIT 0,1" );
                    if ( $check_order ) {
            ...
            static function get_history_bookings( $type = "st_hotel", $offset = 0, $limit = 1, $author = false )
            {
                global $wpdb;
                $where_partner = '';
                if (!in_array('administrator',  wp_get_current_user()->roles)) {
                    $where_partner = " AND partner_id = ". $author. " ";
                }
                $type_booking = 'normal_booking';
                if ( st()->get_option('use_woocommerce_for_booking') == 'on' ) {
                    $type_booking = 'woocommerce';
                }
                $querystr  = "
            ...
            static function get_history_bookings_by_id( $id )
            {
                global $wpdb;
                $table = $wpdb->prefix . 'st_order_item_meta';
                $sql = "SELECT * FROM {$table} WHERE order_item_id=%s";
                $res = $wpdb->get_row( $wpdb->prepare( $sql, $id ) );
                if ( !empty( $res ) && count((array) $res ) > 0 ) {
                    return $res;
                }
                return false;
            }
            static function get_history_bookings_by_wc_order_id( $id )
            {
                global $wpdb;
                $table = $wpdb->prefix . 'st_order_item_meta';
                $sql = "SELECT * FROM {$table} WHERE wc_order_id=%s";
                $res = $wpdb->get_row( $wpdb->prepare( $sql, $id ) );
                if ( !empty( $res ) && count((array) $res ) > 0 ) {
                    return $res;
                }
            ...
            static function get_history_bookings_send_mail( $type = "st_hotel", $offset = 0, $limit = 1, $author = false )
            {
                global $wpdb;
                $where    = "";
            ...
            static function _get_list_item_service_available( $post_type = "st_hotel", $user_id = 0, $page = 1 )
            {
            ...
            function _st_login_popup()
            {
                $page_login = st()->get_option('page_redirect_to_after_login');
                if (!empty($page_login)) {
                    $page_login = get_the_permalink($page_login);
                } else {
                    $page_login = get_the_permalink(STInput::post('post_id', get_the_ID()));
                }
				...
				if ( st()->get_option( 'login_register_enable_captcha', 'off' ) == 'on' ) {
					$recaptcha_url = 'https://www.google.com/recaptcha/api/siteverify';
					$recaptcha_secret = $st_secret_key_captcha;
					$recaptcha_response = isset($_POST['st_captcha_login']) ? $_POST['st_captcha_login'] : '';
					$recaptcha = file_get_contents($recaptcha_url . '?secret=' . $recaptcha_secret . '&response='.$recaptcha_response );
					if ( !$recaptcha ) {
						$recaptcha = STCart::get_content($recaptcha_url . '?secret=' . $recaptcha_secret . '&response='.$recaptcha_response );
					}
					...
					if (isset($recaptcha["score"]) && ($recaptcha["score"] >= 0.5)) {
						...
						echo json_encode([
							'status' => 0,
							'captcha_check' => true,
							'message' => st()->load_template('layouts/modern/common/message', '', ['status' => 'danger', 'message' => $mes_error])
						]);
						die;
            ...
            public function _st_reset_password()
            {
                ...
                if ( $url_confirm_reset ){
                    ...
                    if ( $mail ) {
                        ...
                        echo json_encode( [
                            'status'  => 0,
                            'message' => st()->load_template( 'layouts/modern/common/message', '', [ 'status' => 'danger', 'message' => __( 'Oops something went wrong updating your account.', 'traveler' ) ] )
                        ] );
                    }
                } else {
                    echo json_encode( [
            ...
            function _st_registration_popup()
            {
                ...
                if($theme_layout == 'classic'){
                    ...
                    if (is_wp_error(self::validation())) {
                        ...
                        $notice .= '</div>';
                        $messsage = self::validation()->get_error_message();
                    } else {
                        $register_user = wp_insert_user($userdata);
                        if (!is_wp_error($register_user)) {
                            $class_user = new STUser_f();
                            ...
                            $error = false;
                            $messsage = $class_user->_update_info_user($register_user, false);
                        } else {
                            $notice .= '<div  class="alert alert-danger"><button data-dismiss="alert" type="button" class="close"><span aria-hidden="true">×</span></button>';
                            $notice .= '<strong>' . $register_user->get_error_message() . '</strong>';
                            $notice .= '</div>';
                            $messsage = $register_user->get_error_message();
                        }

                    }

                    echo json_encode(['error' => $error, 'notice' => $notice , 'message' => $messsage]);
                    die;
                }else {
                    check_ajax_referer('st_frontend_security', 'security');
                    $userdata = [
					...
					if ( st()->get_option( 'login_register_enable_captcha', 'off' ) == 'on' ) {
						$recaptcha_url = 'https://www.google.com/recaptcha/api/siteverify';
						$recaptcha_secret = $st_secret_key_captcha;
						$recaptcha_response = isset($_POST['st_captcha_register']) ? $_POST['st_captcha_register'] : '';
						$recaptcha = file_get_contents($recaptcha_url . '?secret=' . $recaptcha_secret . '&response='.$recaptcha_response );
						if ( !$recaptcha ) {
							$recaptcha = STCart::get_content($recaptcha_url . '?secret=' . $recaptcha_secret . '&response='.$recaptcha_response );
						}
                    ...
                    if (is_wp_error(self::validation())) {
                        ...
                        if (!is_wp_error($register_user)) {
                            ...
                            echo json_encode([
                                'status' => 1,
                                'closeText'=> __('Close','traveler'),
                                'message' => st()->load_template('layouts/modern/common/message', '', ['status' => 'success', 'message' => __('Sign Up Successfully', 'traveler')]),
                            ]);
                            die;
                        } else {
                            echo json_encode([
			...
			public function _update_badges( $post_id, $title, $desc, $icon ) {
				if ( !empty( $title ) ) {
					$list_badges       = [];
					if ( !empty( $title ) ) {
						foreach ( $title as $k => $v ) {
							if(!empty($v) && !empty($desc[ $k ])) {
								array_push($list_badges, [
									'title'   => $v,
									'content' => $desc[$k],
									'icon'    => $icon[$k]
								]);
						...
						update_post_meta( $post_id, 'list_badges', $list_badges );
					}else{
						update_post_meta( $post_id, 'list_badges', [] );
					}
				}else{
					update_post_meta( $post_id, 'list_badges', [] );
				}
			...
			public function _update_upsell( $post_id, $type_service, $list_item ) {
				if ( !empty( $type_service ) && !empty( $list_item ) ) {
					...
					update_post_meta( $post_id, 'st_upsell', $st_upsell );
				}
			}
        }
        $user = new STUser_f();
