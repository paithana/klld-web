 ...
 * */
    class STCart
    {
        ...
        static function direct_submit_form()
        {
            ...
            self::set_cart( 'st_cart', unserialize( $cart, [ 'allowed_classes' => false ] ) );
        ...
        static function send_mail_after_booking($order_id = false, $made_by_admin = false, $made_by_partner = false)
        {
            ...
            $email_confirm_for_customer = st()->get_option('enable_email_confirm_for_customer', 'off');
            ...
            if ( $email_to_custommer == 'on' ) {
				...
				if ($status == 'incomplete' && $payment_method == 'st_submit_form' && $email_confirm_for_customer == 'on') {
		...
		static function _send_after_finish_tour_email($order) {
            global $order_id;
            $order_id = $order;
            $to = get_post_meta($order_id, 'st_email', true);
            $subject = sprintf(__('Thanks for booking at %s', 'traveler'), get_bloginfo('title'));
            $item_id = get_post_meta($order_id, 'item_id', true);
            $check = false;
            if ($item_id) {
                $message = "";
                $id_page_email_after_finish_tour = st()->get_option('email_after_finish_tour', '');
                $email_to_custommer = !empty(get_post($id_page_email_after_finish_tour)) ? wp_kses_post(get_post($id_page_email_after_finish_tour)->post_content) : "";
        ...
        {
            return isset($_COOKIE['st_cart']) ? unserialize(stripslashes(gzuncompress(base64_decode($_COOKIE['st_cart']))), [ 'allowed_classes' => false ]) : false;
        ...
        static function get_cart_item()
        {
            ...
            $items = unserialize(stripslashes(gzuncompress(base64_decode($items))), [ 'allowed_classes' => false ]);
        ...
        static function count()
        {
            if (!empty($_COOKIE['st_cart'])) {
                ...
                return count(unserialize(stripslashes(gzuncompress(base64_decode($_COOKIE['st_cart']))), [ 'allowed_classes' => false ]));
            } else {
                return 0;
            }
        }
        ...
        {
            $cart = !empty($_COOKIE['st_cart']) ? unserialize(stripslashes(gzuncompress(base64_decode($_COOKIE['st_cart']))), [ 'allowed_classes' => false ]) : false;
        ...
        static function get_total_with_out_tax($deposit_calculator = false)
        {
            if (isset($_COOKIE['st_cart'])) {
                $cart = unserialize(stripslashes(gzuncompress(base64_decode($_COOKIE['st_cart']))), [ 'allowed_classes' => false ]);
                if (!empty($cart)) {
        ...
        {
            return isset($_COOKIE['st_cart']) ? unserialize(stripslashes(gzuncompress(base64_decode($_COOKIE['st_cart']))), [ 'allowed_classes' => false ]) : [];
        }
        /**
        ...
        {
            return isset($_COOKIE['st_cart']) ? key(unserialize(stripslashes(gzuncompress(base64_decode($_COOKIE['st_cart']))), [ 'allowed_classes' => false ])) : [];
        ...
        {
            if (STInput::post('st_action') == 'apply_coupon' || STInput::post('action') == 'ajax_apply_coupon') {
                // TravelHelper::setcookie('st_cart_coupon', '', time() - 3600);
        ...
        {
            $coupon = (isset($_COOKIE['st_cart_coupon'])) ? unserialize(stripslashes(gzuncompress(base64_decode($_COOKIE['st_cart_coupon']))), [ 'allowed_classes' => false ]) : [];
