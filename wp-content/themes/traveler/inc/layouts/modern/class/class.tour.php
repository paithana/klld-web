...
if ( ! class_exists( 'STTour' ) ) {
	class STTour extends TravelerObject {
		...
		function init() {
			...
			add_action( 'wp_ajax_st_filter_tour_map', [ $this, '__getMapFilterAjax' ] );
            add_action( 'wp_ajax_nopriv_st_filter_tour_map', [ $this, '__getMapFilterAjax' ] );

			add_action( 'after_setup_theme', [ $this, 'st_register_cron' ] );
			add_action( 'st_send_email_after_finish_tour', [ $this, 'sendEmailAfterFinishTour' ] );
		}

		public function st_register_cron() {
			if ( ! wp_next_scheduled( 'st_send_email_after_finish_tour' ) ) {
				wp_schedule_event( time(), 'daily', 'st_send_email_after_finish_tour' );
			}
		}

		public function sendEmailAfterFinishTour() {
			...
			$results = $wpdb->get_results(
				$wpdb->prepare("
					SELECT * FROM $table
					WHERE check_out_timestamp BETWEEN %d AND %d
					AND st_booking_post_type = %s
					AND status = %s
				", $yesterday_start, $yesterday_end, 'st_tours', 'complete' )
			);

			if ( ! empty( $results ) ) {
				foreach ( $results as $booking ) {
					$order_id = $booking->order_item_id;
					$email_after_checkout = st()->get_option('enable_email_after_finish_tour', 'on');
					if ( $email_after_checkout == 'on' ) {
						STCart::_send_after_finish_tour_email( $order_id );
					}
				}
			}
		}
			...
			if ( $query->have_posts() ) {
				while ( $query->have_posts() ) {
					...
					if ( ! empty( $map_lat ) && ! empty( $map_lng ) ) {
						...
						if ( $isNew ) {
							$data_map[$stt]['content_html'] = preg_replace( '/^\s+|\n|\r|\s+$/m', '', stt_elementorv2()->loadView( 'services/tour/components/map-popup' ) );
							$data_map[$stt]['content_adv_html'] = preg_replace( '/^\s+|\n|\r|\s+$/m', '', stt_elementorv2()->loadView( 'services/tour/loop-adv/tour', false, ['post_type' => $post_type_name->label] ) );
						} else {
							$data_map[$stt]['content_html'] = preg_replace( '/^\s+|\n|\r|\s+$/m', '', st()->load_template( 'layouts/modern/tour/elements/map-popup' ) );
							// $data_map[$stt]['content_html'] = preg_replace( '/^\s+|\n|\r|\s+$/m', '', stt_elementorv2()->loadView( 'services/tour/components/map-popup' ) );
							$data_map[$stt]['content_adv_html'] = preg_replace( '/^\s+|\n|\r|\s+$/m', '', stt_elementorv2()->loadView( 'services/tour/loop-adv/tour', false, ['post_type' => $post_type_name->label] ) );
						}
		...
		public function st_format_tour_price() {
			...
			if ( ! empty( $hotel_packages ) ) {
				...
				if ( ! empty( $arr_hotel_temp ) ) {
					...
					foreach ( $arr_hotel_temp as $k => $v ) {
						$sub_hotel_package = json_decode( stripcslashes( $v ), true );
						if ( intval( $arr_hotel_temp[ $k - 1 ] ) > 0 ) {
							if ( ( $k > 0 ) && is_array( $sub_hotel_package ) ) {
								$sub_hotel_package['qty'] = intval( $arr_hotel_temp[ $k - 1 ] );
							}
							if ( is_array( $sub_hotel_package ) ) {
			...
			if ( ! empty( $arr_activity_temp ) ) {
				...
				foreach ( $arr_activity_temp as $k => $v ) {
					$sub_activity_package = json_decode( stripcslashes( $v ), true );
					if ( intval( $arr_activity_temp[ $k - 1 ] ) > 0 ) {
						if ( ( $k > 0 ) && is_array( $sub_activity_package ) ) {
							$sub_activity_package['qty'] = $arr_activity_temp[ $k - 1 ];
						}
						if ( is_array( $sub_activity_package ) ) {
			...
			if ( ! empty( $arr_flight_temp ) ) {
				...
				foreach ( $arr_flight_temp as $k => $v ) {
					$sub_flight_package = json_decode( stripcslashes( $v ), true );
					if ( intval( $arr_flight_temp[ $k + 1 ] ) > 0 ) {
						if ( is_array( $sub_flight_package ) ) {
				...
				while ( $query->have_posts() ) {
					...
					if ( function_exists( 'check_using_elementor' ) && check_using_elementor() ) {
						...
						if ( $version == 'elementorv2' ) {
							$classes = '';
							if ( $layoutRequest == 6 ) {
								$classes = ( $st_style == 'grid' ) ? 'col-lg-4 col-md-6 col-12 item-service' : 'col-lg-12 col-md-12 col-12 item-service';
							...
							} elseif ($layoutRequest == 8) {
								$classes = ( $st_style == 'grid' ) ? ' col-12 col-md-6 col-lg-6  item-service' : ' col-12 item-service';
							}else {
								$classes = ( $st_style == 'grid' ) ? ' col-12 col-md-6 col-lg-3  item-service' : ' col-12 item-service';
							}

							if ( $formatRequest == 'halfmap' ) {
						...
						if ( STInput::request( 'format' ) == 'halfmap' ) {
			...
			if ( $check = STInput::get( 'orderby' ) ) {
				...
				if ( empty( $wp_query->query['orderby'] ) ) {
					...
					if ( ! empty( $is_featured ) && $is_featured == 'on' ) {
						$orderby = 'tb_st_tours.is_featured desc, ' . $wpdb->posts . '.post_date desc';
					} else {
		...
		function do_add_to_cart() {
			...
			// Pickup location
			$pickup         = trim( STInput::request( 'pickup', '' ) );
			$data['pickup'] = $pickup;
			...
			if ( $pass_validate ) {
				...
				if ( $pass_validate ) {
					STCart::add_cart( $item_id, $number, $data['sale_price'], $data );
				}
			}
		...
		static function get_price_html( $post_id = false, $get = false, $st_mid = '', $class = '', $hide_title = true ) {
			...
			*/
			...
			$meta_key = $price_type == 'person' ? 'adult_price' : 'price';
			$where    = "where 1=1 and ( {$wpdb->prefix}{$post_type}.post_id = {$post_id} and `check_in` >= {$check_in} ) AND status = 'available'";
			$sql      = "SELECT
                    min(CAST({$meta_key} as DECIMAL(15,6))) as min
                FROM
