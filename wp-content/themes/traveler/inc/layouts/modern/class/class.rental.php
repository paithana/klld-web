...
if (!class_exists('STRental')) {
    class STRental extends TravelerObject
    {
        ...
        public function check_price_ajax(){
			...
			$booking_min_day = $this->get_data_rental_by_checkin($rental_origin, strtotime($check_in));
            if ($booking_min_day) {
				$booking_min_day = $booking_min_day[0]->booking_min_day;
            ...
            $price_sale = STPrice::getSalePrice($rental_origin, strtotime($check_in), strtotime($check_out), $adult_number, $child_number);
...
//                    ";
        ...
        function do_add_to_cart()
        {
			...
			$adult_number = intval( STInput::request( 'adult_number', 0 ) );
            $item_price = STPrice::getRentalPriceOnlyCustomPrice($rental_origin, strtotime($check_in), strtotime($check_out), $adult_number, $child_number);
            $extras = STInput::request('extra_price', []);
            $numberday = STDate::dateDiff($check_in, $check_out);
            $extra_price = STPrice::getExtraPrice($rental_origin, $extras, 1, $numberday);
            $price_sale = STPrice::getSalePrice($rental_origin, strtotime($check_in), strtotime($check_out), $adult_number, $child_number);
			...
			$price_item_caculator = $this->get_data_rental_availability($rental_origin, strtotime($check_in), strtotime($check_out), $adult_number, $child_number);
			if (get_post_meta($rental_origin, 'rental_price_by', true) == 'person') {
		...
		public static function get_data_rental_by_checkin($room_id = '', $check_in = '') {
			global $wpdb;
            $where = "
                and check_in = %d";
            $sql = "
                ...
                {$where}
                ";
            $results = $wpdb->get_results($wpdb->prepare($sql, [$room_id, $check_in]));
            return $results;
		}

		function get_data_rental_availability($room_id = '', $check_in = '', $check_out = '', $adult_number = 1, $child_number = '')
        {
            ...
            $sale_by_day = array();
            $sale_count_date = 0;
            if (get_post_type($room_id) == 'st_rental') {
                $price_by_per_person = get_post_meta($room_id, 'rental_price_by', true);
                if ($price_by_per_person == 'person') {
                ...
                $custom_price = AvailabilityHelper::_getdataRental($room_origin_id, $check_in, $check_out);
        ...
        function _get_where_query($where)
        {
             ...
             */
            if (STInput::request('location_id')) {
                $st_search_args['location_id'] = esc_attr( STInput::request('location_id') );
            }
            if (!empty($st_search_args['st_location'])) {
                if (empty($st_search_args['only_featured_location']) or $st_search_args['only_featured_location'] == 'no')
                    $st_search_args['location_id'] = esc_attr( $st_search_args['st_location'] );
            }
            if (isset($st_search_args['location_id']) && !empty($st_search_args['location_id'])) {
                $location_id = esc_attr( $st_search_args['location_id'] );
                $where = TravelHelper::_st_get_where_location($location_id, ['st_rental'], $where);
            } elseif (isset($_REQUEST['location_name']) && !empty($_REQUEST['location_name'])) {
                $location_name = esc_html( STInput::request('location_name', '') );
            ...
            $check_out = esc_html( STInput::get('end', '') );
            if (empty($check_out)) {
            ...
            if (!empty($check_in) && !empty($check_out)) {
                ...
                if ($avai_check === 'on') {
                    ...
                    if (is_array($list_rental) and !empty($list_rental)) {
                        ...
                        // $where .= $wpdb->prepare( " AND {$wpdb->posts}.ID NOT IN (%s)", $list_rental );
						$placeholders = implode( ',', array_fill( 0, count( $list_rental ), '%d' ) );
                        $where .= $wpdb->prepare( " AND {$wpdb->posts}.ID NOT IN ($placeholders)", ...$list_rental );
                    }
					$where .= $wpdb->prepare( " AND CAST(tb.rentals_booking_period AS UNSIGNED) <= %d", $period );
                }

            }
            if ($stars = STInput::get('star_rate')) {
                $stars = esc_html( STInput::get('star_rate', 1) );
                $stars = explode(',', $stars);
                $all_star = [];
                ...
                $list_star = implode(',', array_fill(0, count($all_star), '%f'));
                if ($list_star) {
                    $where .= $wpdb->prepare( " AND (tb.rate_review IN ($list_star))", ...$all_star );
                }
            }
            ...
            if (STInput::get('adult_number')) {
				$adult_number = esc_html( STInput::get('adult_number') );
				$where .= $wpdb->prepare( " AND tb.rental_max_adult >= %d", $adult_number );
            }
            if (STInput::get('child_number')) {
				$child_number = esc_html( STInput::get('child_number') );
				$where .= $wpdb->prepare( " AND tb.rental_max_children >= %d", $child_number );

            }
            if (isset($_REQUEST['range']) and isset($_REQUEST['location_id'])) {
                $range = esc_html( STInput::get('range', '0;5') );
                $rangeobj = explode(';', $range);
                $range_min = $rangeobj[0];
                $range_max = $rangeobj[1];
                $location_id = esc_html( STInput::request('location_id') );
				...
				$where .= $wpdb->prepare(
					"AND $wpdb->posts.ID IN (
						SELECT ID FROM (
							SELECT $wpdb->posts.*,
								( 6371 * acos(
									cos( radians(%f) ) * cos( radians( mt1.meta_value ) ) *
									cos( radians( mt2.meta_value ) - radians(%f) ) +
									sin( radians(%f) ) * sin( radians( mt1.meta_value ) )
								) ) AS distance
							FROM $wpdb->posts
							INNER JOIN $wpdb->postmeta AS mt1 ON $wpdb->posts.ID = mt1.post_id
							INNER JOIN $wpdb->postmeta AS mt2 ON $wpdb->posts.ID = mt2.post_id
							WHERE mt1.meta_key = 'map_lat'
							AND mt2.meta_key = 'map_lng'
							AND $wpdb->posts.post_status = 'publish'
							AND $wpdb->posts.post_type = %s
							AND $wpdb->posts.post_date < NOW()
							GROUP BY $wpdb->posts.ID
							HAVING distance >= %f AND distance <= %f
							ORDER BY distance ASC
						) AS st_data
					)",
					$map_lat, $map_lng, $map_lat, $post_type, $range_min, $range_max
				);
            }

            //Query nearby lat/lng
            if (isset($_REQUEST['location_lat']) && isset($_REQUEST['location_lng'])) {
                $location_lat = esc_html( $_REQUEST['location_lat'] );
                $location_lng = esc_html( $_REQUEST['location_lng'] );
                $location_distance = isset($_REQUEST['location_distance']) ? esc_html( $_REQUEST['location_distance'] ) : 500;
                if ($location_lat && $location_lng) {
					...
					$where .= $wpdb->prepare(
						...
						$location_lat, $location_lng, $location_lat, 'st_rental', $location_distance
					);
            ...
            if (!empty($st_search_args['only_featured_location']) and !empty($st_search_args['featured_location'])) {
                $featured = esc_html( $st_search_args['featured_location'] );
                if ($st_search_args['only_featured_location'] == 'yes' and is_array($featured)) {
                    if (is_array($featured) && count($featured)) {
						...
						foreach ($featured as $item) {
							$where_conditions[] = "tb.multi_location LIKE %s";
							$params[] = "%_{$item}_%";
						}

						$placeholders = implode(',', array_fill(0, count($featured), '%d'));
						$where_conditions[] = "tb.id_location IN ($placeholders)";
						$params = array_merge($params, $featured);
