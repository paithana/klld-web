<?php
    ...
    if ( !class_exists( 'STPaymentGateways' ) ) {
        class STPaymentGateways
        {
             ...
             * @return bool
             */
            static function gateway_success_page_validate( $id = false , $order_id = '' )
            {
                $all = self::get_payment_gateways();
                        ...
                        if($order_id ) {
                            ...
                            if ($status == 'incomplete') {
                                if ($value->is_check_complete_required()) {
                                    $r = $value->check_complete_purchase($order_id);
                                ...
                                if ($r && ($payment !== 'st_submit_form') ) {
                                    if (isset($r['status'])) {
                                        ...
                                        if ($r['status']) {
                                            ...
                                            $status = 'complete';
											// Send mail complete success
                                            STCart::send_mail_after_booking($order_id, true);
                                            do_action('st_booking_change_status', 'complete', $order_id, $value->getGatewayId());
            ...
            static function do_checkout( $gateway, $order_id )
            {
				...
				// Send mail incomplete after booking
				STCart::send_mail_after_booking( $order_id, true );
				do_action( 'st_traveler_after_booking', 'incomplete', $order_id );
