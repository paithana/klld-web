    ...
    if ( !class_exists( 'STReview' ) ) {
        class STReview
        {
            ...
            function init()
            {
                ...
                if ( !is_admin() ) {
                    add_filter( 'comments_clauses', [ $this, 'st_comments_clauses' ], 9, 2 );
                    add_filter( 'wp_count_comments', [ $this, 'st_wp_count_comments' ], 9, 3 );
                }
                add_filter( 'notify_moderator', [ $this, '_check_notify_moderator' ] );
                add_filter( 'notify_post_author', [ $this, '_check_notify_post_author' ] );

                //Only sent mail approve to admin not author
                add_filter( 'comment_notification_recipients', '__return_empty_array', 10, 1 );
                add_filter( 'comment_moderation_recipients', [ $this, 'st_admin_comment_emails' ], 10, 1 );
             ...
             */
             ...
             function st_admin_comment_emails($emails){
                {
            ...
            function like_review()
            {
                ...
                if ( $this->find_by( $comment_id ) ) {
                    ...
                    if ( is_user_logged_in() ) {
                        ...
                        $meta_comment = get_user_option( "_liked_comments", $user_id ); // comments ids from user meta
                        $meta_user = get_comment_meta( $comment_id, "_user_liked" ); // user ids from comment meta
                        $liked_comments = null; // setup array variable
                        $liked_user    = null; // setup array variable
                        if ( !empty($meta_comment) && count( $meta_comment ) != 0 ) { // meta exists, set up values
                            $liked_comments = $meta_comment;
                        }
                        if ( !is_array( $liked_comments ) ) // make array just in case
                            $liked_comments = [];
                        if ( count( $meta_user ) != 0 ) { // meta exists, set up values
                            $liked_user = $meta_user[ 0 ];
                        }
                        if ( !is_array( $liked_user ) ) // make array just in case
                            $liked_user = [];
                        $liked_comments[ 'comment-' . $comment_id ] = $comment_id; // Add comment id to user meta array
                        $liked_user[ 'user-' . $user_id ]          = $user_id; // add user id to comment meta array
                        $user_likes                                 = count( $liked_comments ); // count user likes
                        if ( !$this->check_like( $comment_id ) ) { // like the comment
                            update_comment_meta( $comment_id, "_user_liked", $liked_user ); // Add user ID to comment meta
                            ...
                            $uid_key = array_search( $user_id, $liked_user ); // find the key
                            unset( $liked_comments[ $pid_key ] ); // remove from array
                            unset( $liked_user[ $uid_key ] ); // remove from array
                            $user_likes = count( $liked_comments ); // recount user likes
                            update_comment_meta( $comment_id, "_user_liked", $liked_user ); // Remove user ID from comment meta
                            update_comment_meta( $comment_id, "_comment_like_count", --$comment_like_count ); // -1 count comment meta
            ...
            { // test if user liked before
                if ( is_user_logged_in() ) { // user is logged in
                    $user_id     = get_current_user_id(); // current user
                    $meta_user  = get_comment_meta( $comment_id, "_user_liked" ); // user ids from comment meta
                    $liked_user = ""; // set up array variable
                    if ( count( $meta_user ) != 0 ) { // meta exists, set up values
                        $liked_user = $meta_user[ 0 ];
                    }
                    if ( !is_array( $liked_user ) ) // make array just in case
                        $liked_user = [];
                    if ( in_array( $user_id, $liked_user ) ) { // True if User ID in array
                        return true;
                    }
                    return false;
            ...
            function save_comment_meta_data( $comment_id )
            {
                ...
                if(in_array($current_post_type,$all_postype)){
                    ...
                    if ( get_option( 'comment_moderation' ) == '1' ) $array[ 'comment_approved' ] = 0;
                    if ( is_super_admin() ) {
                        $array[ 'comment_approved' ] = 1;
                    }
